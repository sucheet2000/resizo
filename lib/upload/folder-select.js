 ...
 * Intl.Collator on every call, and this comparator runs O(n log n) times over a
 * whole picked folder. Measured with a per-call collator: 10,000 paths took
 * 354 ms and 100,000 took 3,837 ms — seconds of frozen tab at the moment of the
 * pick, on exactly the tens-of-thousands folder FOLDER_SCAN_LIMIT exists for.
 * Hoisted, the same 10,000 sort is a few milliseconds.
 */
...
function byPath(a, b) {
    ...
    return pathCollator.compare(left, right);
}
...
export async function scanFolderPick({
    ...
    // SORT, THEN TRUNCATE — in that order, because the notice this produces
    // says "the first 1000 by name were checked". Slicing first truncated in
    // whatever order the directory input enumerated (creation order, in a real
    // walk) and sorted only the survivors, so the sentence was false for every
    // folder over the limit: a 1200-file folder enumerated backwards handed
    // back Trip/00200.jpg onwards while Trip/00000.jpg was never even opened.
    const scanned = visible.slice().sort(byPath).slice(0, scanLimit);
