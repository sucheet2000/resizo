...
import { CONVERT_OUTPUT_FORMATS, DEFAULT_QUALITY as ENGINE_DEFAULT_QUALITY } from '@/lib/limits';
import { STATUS, isCancellation, readBlobHead, readBlobSize, summarize } from '@/lib/upload/batch';

/** The three the engine can write. Read from the registry, never re-typed. */
export const OUTPUT_FORMATS = CONVERT_OUTPUT_FORMATS;
...
/**
 * The two formats a quality dial means anything for. PNG here is lossless with
 * no quantiser at all, so a slider on it would be a control that does nothing —
 * the panel says so out loud instead of offering one.
 */
...
export function createConvertProcessor({
    ...
    return async function processOne({
        ...
        try {
            sourceAlpha = (await readSize(file))?.hasAlpha ?? null;
        } catch (error) {
            if (isCancellation(error, signal)) throw error;
            // Unknown, which is not the same as false: it relaxes the alpha
            // rule below rather than inventing an answer for it.
            sourceAlpha = null;
        }
        ...
        async function verify(result) {
            ...
            const report = validate(bytes, {
                ...
                transparency: target === 'jpeg' ? 'removed' : (sourceAlpha === true ? 'kept' : null),
            });
            ...
            if (target !== 'jpeg' && sourceAlpha === true && size.hasAlpha !== true) return null;

            const flattened = target === 'jpeg' && sourceAlpha === true;
            ...
            return {
                ...
                outputFormat: target,
                flattened,
                background: flattened ? background : null,
