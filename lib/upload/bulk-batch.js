...
export async function processBatch({
    ...
    // subtracts is the largest PROCESSABLE file's and that does not change as
    // the run goes.
    //
    // `operation` and `device` are passed straight through rather than spread in
    // conditionally. A conditional spread reads as though omitting the argument
    // is a different code path, and that is how a ceiling ends up depending on
    // whether an optional argument was supplied; `undefined` already selects
    // capability.js's own default parameter, so the two are identical and only
    // one of them can be misread.
