 ...
 */
import { BULK_OUTPUT_FORMATS, MAX_DIMENSION } from '@/lib/limits';
...
/** "JPEG, PNG or WebP" — the batch list as a person reads it, never re-typed. */
const BATCH_FORMAT_LIST = (() => {
    const labels = BULK_OUTPUT_FORMATS.map(formatLabel);
    return `${labels.slice(0, -1).join(', ')} or ${labels[labels.length - 1]}`;
})();

/**
 * A batch writes fewer formats than one image can (BULK_OUTPUT_FORMATS says
 * why AVIF is the gap). The panel resolves "Same as the original" before the
 * batch starts, so an AVIF source kept as AVIF arrives here as a concrete
 * 'avif' — and is refused per row, in words, before the gate or the engine is
 * asked. The other rows still finish and still zip.
 */
function batchFormatRefusal(format) {
    return `A batch writes ${BATCH_FORMAT_LIST} only, so this image cannot be written as ${formatLabel(format)} here. `
        + 'Resize it on its own, or pick one of those formats for the batch.';
}
...
export function createFileProcessor({ op = 'resize', process = processImage } = {}) {
    return async function processOne({
        ...
        if (fields?.format && !BULK_OUTPUT_FORMATS.includes(fields.format)) {
            return { ok: false, error: batchFormatRefusal(fields.format) };
        }
