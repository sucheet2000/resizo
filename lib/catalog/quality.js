...
export function stuffingReport(intent) {
    const body = normalise(intentCopy(intent, { part: 'body' }).join(' '));
    const all = words(body);

    const shares = slugWords(intent?.slug).map((word) => {
        const hits = all.filter((candidate) => candidate === word).length;
        return { word, hits, share: all.length > 0 ? hits / all.length : 0 };
    });

    return {
        phrase: normalise(intent?.h1),
        phraseCount: countPhrase(body, normalise(intent?.h1)),
