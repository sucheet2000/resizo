 ...
 */
import { APPLICATION_PRESETS } from './application-presets';
import { PHYSICAL_UNITS, validateApplicationPresets } from './application-presets/validate';
import { validateBehaviour } from './behaviour';
import { CATEGORIES } from './categories';
import { ICON_SOURCES, validateIconSources } from './icon-sources';
import { inlineLinks, isLinkTarget } from './inline';
import { INTENTS } from './intents';
import { plainText } from './inline';
import { PAPER_SIZES, paperMillimetres } from './paper-sizes';
import { ASPECT_RATIOS, SOCIAL_PRESETS, validatePresets } from './presets';
import { qualityProblems } from './quality';
import { intentLinks } from './relations';
...
function blockProblem(block) {
    ...
    if (block.type === 'figure') return figureProblem(block);

    return `unknown block type "${block?.type}"`;
}

/**
 * A demonstration image, or a before/after pair of them.
 *
 * Every constraint here is a way the block renders wrong rather than a style
 * preference. `src` is pinned to /demos/ because a figure is a file this repo
 * ships and checks, not a hotlink; width and height are required because
 * next/image needs both and a page without them reflows as each picture
 * arrives; the alt text and the caption have length floors because "before"
 * and "after" tell a screen-reader user nothing about what changed, and the
 * caption is the only part of a figure a text-only crawler can read.
 */
const FIGURE_ALT_LENGTH = 20;

const FIGURE_CAPTION_LENGTH = 20;

function figureImageProblem(image, which) {
    if (!image || typeof image !== 'object') return `${which} must be an image object`;
    if (typeof image.src !== 'string' || !image.src.startsWith('/demos/')) {
        return `${which} needs a src under /demos/`;
    }
    if (!isText(image.alt) || image.alt.trim().length < FIGURE_ALT_LENGTH) {
        return `${which} needs alt text of at least ${FIGURE_ALT_LENGTH} characters describing what the picture shows`;
    }
    for (const side of ['width', 'height']) {
        if (!Number.isSafeInteger(image[side]) || image[side] <= 0) {
            return `${which} needs a whole-number ${side} in pixels`;
        }
    }
    return null;
}

function figureProblem(block) {
    const pair = block.before !== undefined || block.after !== undefined;
    const single = block.image !== undefined;

    if (pair === single) return 'a figure needs either one image or a before/after pair, not both and not neither';

    const found = pair
        ? (figureImageProblem(block.before, 'the before image') ?? figureImageProblem(block.after, 'the after image'))
        : figureImageProblem(block.image, 'the image');
    if (found) return found;

    if (!isText(block.caption) || block.caption.trim().length < FIGURE_CAPTION_LENGTH) {
        return `a figure needs a caption of at least ${FIGURE_CAPTION_LENGTH} characters`;
    }
 ...
 * preconfigure. `links` is the site-link graph — lib/catalog/relations.js in
 * production, and injectable so a test can prove the related-pages rule fires
 * without inventing a second tool registry.
 */
export function validateIntent(intent, { tools = TOOLS, links = intentLinks } = {}) {
    const problems = [];
    ...
    // The content-quality contract, which applies to a page that is going in
    // the index and not to a draft kept out of it.
    if (intent?.indexable === true) {
        if (!isTextList(intent?.limitations) || intent.limitations.length === 0) {
            problem('intent-limitations-missing', `intent "${subject}" states no limits — an indexable page has to say where it stops`);
        }

        const parent = tools.find((candidate) => candidate.slug === intent?.tool)?.href;
        const related = (links(intent) ?? []).filter((path) => path !== parent);
        if (related.length === 0) {
            problem('intent-related-missing', `intent "${subject}" links nowhere on the site except its parent tool`);
        }

        for (const found of qualityProblems(intent, { tools })) {
            problem(found.code, `intent "${subject}": ${found.message}`);
        }
    }
 ...
 * closest real pair scores 0.092, a number-swapped clone 1.000 — see
...
export function validateIntents(intents, { tools = TOOLS, links = intentLinks } = {}) {
    const problems = intents.flatMap((intent) => validateIntent(intent, { tools, links }));
...
/**
 * A millimetre figure and the size it was converted from may differ by this
 * much. lib/catalog/paper-sizes.js stores the conversion rounded to a
 * ten-thousandth of a millimetre so 6 in reads as 152.4 rather than as
 * 152.39999999999998, and this window is wide enough to let that rounding
 * through and far too narrow to let a typed 152 pass for it.
 */
const PAPER_MM_TOLERANCE = 1e-3;

/**
 * Every way a paper size can be wrong about itself.
 *
 * The rule that matters is the last one: the width and the height are stored
 * in the unit the sheet is named in, and the millimetre figures beside them
 * are a conversion. Two numbers describing one length is exactly how "2 x 2
 * inches (51 x 51 mm)" ends up two pixels wrong at 300 DPI, so they are
 * checked against each other here rather than trusted as separate facts.
 *
 * @param {Array} [papers]  defaults to the shipped registry
 * @returns {Array<{ code: string, subject: string, message: string }>}
 */
export function validatePaperSizes(papers = PAPER_SIZES) {
    const problems = [];
    const seen = new Set();

    for (const paper of papers) {
        const subject = paper?.id ?? '(unnamed paper size)';
        const problem = (code, message) => problems.push({ code, subject, message: `paper size "${subject}": ${message}` });

        if (!paper || typeof paper !== 'object') {
            problems.push({ code: 'paper-size-malformed', subject, message: 'a paper size must be an object' });
            continue;
        }

        if (!isText(paper.id)) problem('paper-size-fields-missing', 'a paper size needs an id');
        if (seen.has(paper.id)) problem('paper-size-id-duplicate', 'this id is declared more than once');
        seen.add(paper.id);

        if (!isText(paper.label)) problem('paper-size-fields-missing', 'a paper size needs a label the select can show');

        const sized = ['width', 'height', 'widthMm', 'heightMm']
            .every((side) => Number.isFinite(paper[side]) && paper[side] > 0);
        if (!sized) {
            problem('paper-size-dimensions-invalid', 'width, height, widthMm and heightMm must all be numbers above zero');
        }

        if (!PHYSICAL_UNITS.includes(paper.unit)) {
            problem(
                'paper-size-unit-invalid',
                `a paper size is named in ${PHYSICAL_UNITS.join(', ')}, not ${JSON.stringify(paper.unit)}`,
            );
            continue;
        }
        ...
        for (const [side, mm] of [['width', 'widthMm'], ['height', 'heightMm']]) {
            const converted = paperMillimetres(paper[side], paper.unit);
            if (Math.abs(converted - paper[mm]) > PAPER_MM_TOLERANCE) {
                problem(
                    'paper-size-unit-invalid',
                    `${paper[side]} ${paper.unit} is ${converted} mm, but ${mm} says ${paper[mm]} — `
                        + 'the millimetres are a conversion of the stated unit, not a second number',
                );
            }
        }
    }
...
export function validateCatalog({
    ...
    applicationPresets = APPLICATION_PRESETS,
    ...
    // What each tool changes about a file is a claim on every one of its pages,
    // so a tool that ships without one — or one whose entry outlives its tool —
    // fails the build here rather than rendering an empty spec block.
    problems.push(...validateBehaviour(tools));
    // A platform size that cites a source cites a real, dated, first-party one.
    problems.push(...validatePresets(presets));
    // A government requirement cites the authority itself, states one shape
    // rather than two, and converts its own unit — see
    // lib/catalog/application-presets/validate.js for why this is stricter
    // than the rule above it.
    problems.push(...validateApplicationPresets(applicationPresets));
    // Every pixel size in the favicon package comes out of somebody else's
    // published document, so an entry with no citation — or one that calls a
    // recommendation a requirement — fails the build rather than reaching the
    // page that prints it.
    problems.push(...validateIconSources(iconSources));
    // A sheet of paper states its size once, in the unit it is named in. The
    // millimetres beside it are a conversion and are held to it.
    problems.push(...validatePaperSizes(paperSizes));
