...
export function validateApplicationPresets(presets = APPLICATION_PRESETS) {
    ...
    for (const preset of presets) {
        ...
        if (!Array.isArray(preset.formats) || preset.formats.length === 0
            || !preset.formats.every((format) => FIT_OUTPUT_FORMATS.includes(format))) {
            problem('app-preset-fields-missing', `formats must be a non-empty list of ${FIT_OUTPUT_FORMATS.join(', ')}`);
        }
