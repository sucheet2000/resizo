 ...
 *
 *   One tool writes nothing at all.  /image-metadata-viewer reads a file and
 *     prints what is stored in it. No decoder runs, no encoder runs and no
 *     image comes back, so every row is the same answer — read, shown,
 *     unchanged — and the entry's note carries the sentence a reader who
 *     wants the metadata gone actually needs.
 *
...
export const BEHAVIOUR_VALUES = {
    pixels: {
        ...
        values: {
            ...
            /**
             * Not "copied", which describes bytes arriving somewhere else.
             * /image-metadata-viewer produces no file: the bytes are walked
             * for the blocks around the picture and the picture itself is
             * never opened, so there is nothing to copy them into.
             */
            untouched: { detail: 'Not decoded and not written — the file is only read', text: 'Pixels untouched' },
    ...
    exif: {
        ...
        values: {
            ...
            read: { detail: 'Read and shown, never changed', text: 'EXIF read only' },
    ...
    gps: {
        ...
        values: {
            ...
            read: { detail: 'Read and shown, never changed', text: 'GPS read only' },
    ...
    xmp: {
        ...
        values: {
            ...
            read: { detail: 'Read and shown, never changed', text: 'XMP read only' },
    ...
    icc: {
        ...
        values: {
            ...
            read: { detail: 'Read and shown, never changed', text: 'ICC profile read only' },
    ...
    dpi: {
        ...
        values: {
            ...
            read: { detail: 'Read and shown, never changed', text: 'DPI record read only' },
            /**
    ...
    transparency: {
        ...
        values: {
            ...
            read: { detail: 'Read and shown, never changed', text: 'Transparency read only' },
            flattened: {
            ...
            /**
             * The honest answer for a tool whose output format is fixed and
             * whose BACKGROUND is the choice. /favicon-generator writes PNGs
             * and an .ico full of PNGs and nothing else, so 'depends' would
             * name JPEG on a page that cannot produce one and point at a format
             * control the panel does not have — while 'kept' would be false the
             * moment somebody picks a colour, which Apple's own guidance asks
             * them to do for a home-screen icon. Same shape of fork as
             * `dpi: 'optional'` two fields up, and the same name for it.
             */
            optional: {
                detail: 'Kept in every PNG and in favicon.ico — unless you choose a background colour, and then every icon is composited onto it',
                text: 'Transparency kept unless you choose a background',
            },
...
export const BEHAVIOUR = {
    ...
    convert: {
        ...
        transparency: 'depends',
        /**
         * AVIF is the only source format whose decode changes the picture
         * before this tool is handed anything. A file may store 10 or 12 bits
         * a channel, a wide gamut or an HDR transfer curve; the browser's own
         * decoder — the only AVIF decoder in this build, see
         * docs/rfc/avif-codec-review-2026-09-11.md — returns 8-bit sRGB in
         * every one of those cases, so the reduction has already happened by
         * the time an encoder sees a sample. No row in BEHAVIOUR_FIELDS asks
         * about the INPUT, and a plain `note` here would print this on
         * /png-to-jpg, where none of it is true of anything. So it resolves
         * from `preset.from` and reaches the two AVIF pages only.
         */
        sourceNotes: {
            avif: 'An AVIF is opened by your browser\'s own decoder rather than by code this page '
                + 'downloads, and that decoder hands over 8-bit sRGB samples whatever the file stored: a '
                + '10- or 12-bit picture arrives reduced to 8-bit, and a wide-gamut or HDR one arrives '
                + 'converted to ordinary sRGB. What you download is an exact record of that decode. A file '
                + 'holding an animation is refused before any of it happens.',
        },
    },
    /**
    ...
    /**
     * The twelfth re-encoder, and the only one that hands back a set of files
     * rather than one. Everything up to the encode is the path the eleven above
     * take — decode once, crop or fit to a square, resample to each size,
     * encode — so the five metadata rows read the same and for the same reason.
     * The DPI row is plain 'removed' rather than the fitters' 'optional':
     * nothing here asks for a print size, and an icon has no physical size to
     * claim.
     *
     * The transparency row is the one that could not be borrowed. Every raster
     * this tool writes is a PNG, favicon.ico's three payloads included, so
     * there is no output format for a 'depends' to depend on — the fork is the
     * background control instead, and `optional` is the value that says so.
     */
    'favicon-generator': {
        pixels: 'reencoded',
        exif: 'removed',
        ...
        note: 'Transparency is kept in every PNG and in favicon.ico unless you choose a background; then every icon is composited onto it. Nothing else from the source survives the encode.',
    },
    'change-image-dpi': {
        pixels: 'copied',
        exif: 'rewritten',
    ...
    /**
     * The only tool on the site whose output is a page rather than a file. It
     * walks the same containers /remove-image-metadata walks — the exports of
     * lib/image-client/metadata-strip.js are a report and a rewrite, and this
     * page asks for the report — so nothing here is decoded, nothing is
     * encoded and no byte of the visitor's file is written anywhere.
     *
     * Every row therefore reads `read` rather than `kept`. "Kept" is a claim
     * about a file that came back, and none did; a visitor who reads "EXIF
     * kept" beside a tool that produces no image would reasonably think one
     * had been written for them with the EXIF still in it.
     */
    'image-metadata-viewer': {
        ...
        dpi: 'read',
        transparency: 'read',
        note: 'Nothing is written: this tool produces no image. To take metadata out, use Remove Image Metadata.',
    },
...
/**
 * The source format an intent's preset pins, or null when the page takes
 * whatever is dropped on it.
 *
 * /convert is the only tool whose preset names one: its `from` narrows the
 * drop zone to a single format (see the `accept` memo in
 * app/(tools)/convert/ConvertTool.js), which is what makes a note about how
 * that format is READ true of the whole page rather than of one file in it.
 */
function pinnedSource(toolSlug, preset) {
    if (toolSlug !== 'convert' || !preset || typeof preset !== 'object') return null;

    const named = preset.from;
    if (typeof named !== 'string' || named.trim() === '') return null;
...
export function behaviourFor(toolSlug, preset = undefined) {
    ...
    const sourceNote = entry.sourceNotes?.[pinnedSource(toolSlug, preset)] ?? null;

    return { rows, note: sourceNote ?? entry.note ?? null };
}
...
export function validateBehaviour(tools = []) {
    ...
    for (const [slug, entry] of Object.entries(BEHAVIOUR)) {
        ...
        for (const [format, note] of Object.entries(entry.sourceNotes ?? {})) {
            if (typeof note !== 'string' || note.trim() === '') {
                problem(
                    'behaviour-value-invalid',
                    slug,
                    `behaviour "${slug}" has a ${format} source note that is not a sentence`,
                );
            }
        }
    }
