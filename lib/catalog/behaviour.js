 ...
 *
 *   Seven tools re-encode.  resize, crop, compress, convert, heic, the
 *     signature resizer and the passport photo tool all run decode →
 *     (transform) → encode, and lib/image-client/encode.js writes a file from
 *     an ImageData and nothing else. There is no path by which an EXIF block, a
 *     GPS tag, an XMP packet, an ICC profile or a density record reaches an
 *     encoder: none of them is a pixel. So all five metadata rows are "removed"
 *     for all seven, and that is a consequence of the architecture rather than
 *     a promise someone made. The passport tool differs on one row only: it can
 *     write a resolution back AFTER the encode, the way /change-image-dpi does,
 *     when the requirement it was given asks for one.
 *
