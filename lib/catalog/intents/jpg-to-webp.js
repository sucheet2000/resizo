...
const jpgToWebp = {
    ...
    changes: {
        does: [
            'Decodes the JPEG and writes a lossy WebP at quality 80, which usually comes back lighter than the JPEG it was made from.',
            'Costs a second lossy generation, since the picture is compressed again rather than repackaged into a new container.',
            'Reports the real before and after byte counts, measured on your own file rather than quoted from an average.',
        ],
        doesNot: [
            'Does not change the pixel dimensions, so [resize first](/resize) if the photo is larger than it will ever be shown.',
            'Does not produce an animated WebP — one still frame goes in and one still frame comes out.',
            'Does not carry EXIF or GPS into the WebP, including the camera and location tags the JPEG was holding.',
        ],
    },
    ...
    limitations: [
        'This page takes one JPEG at a time, up to 20 MB, so a gallery is converted photograph after photograph rather '
            + 'than in a single pass over a folder.',
        'How much weight comes off depends on the picture: a busy, high-contrast frame can come back only slightly '
            + 'smaller, and the measured figures in the panel are the only reliable number.',
        'The longest side may not exceed 8000 pixels, and an image past that is turned down before any pixels are '
            + 'allocated rather than partway through the encode.',
    ],
