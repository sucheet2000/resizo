...
import { MAX_BULK_FILES, MAX_DIMENSION, MAX_FILE_SIZE, MAX_SCALE_PERCENT } from '@/lib/limits';
...
const resizeJpg = {
    ...
    changes: {
        does: [
            'Redraws the photograph at the width and height you type, with one field filling in the other '
                + 'while the ratio lock is ticked.',
            'Writes a brand-new JPEG at quality 80, one more generation of lossy compression on top of '
                + 'whatever the original already carried.',
            'Fills the exact box when you set both sides yourself, scaling to cover it and trimming the '
                + 'overflow from the edges.',
        ],
        doesNot: [
            'Leaves the file type alone: the format select reads Same as the original, so a JPEG in is a '
                + 'JPEG out with its extension intact.',
            'Carries no EXIF or GPS data across from the camera, because the output is rebuilt from raw '
                + 'pixels and nothing else.',
            'Offers no quality slider — 80 is fixed here, so [compress to a target size]'
                + '(/compress-image-to-200kb) when the bytes are the point.',
        ],
    },
    ...
    limitations: [
        'Only JPEG, PNG and WebP open here, so an iPhone HEIC has to pass through '
            + '[HEIC to JPG](/heic-to-jpg) before this page will take it.',
        `Percent mode stops at ${MAX_SCALE_PERCENT}%, so a single pass cannot enlarge a photograph more `
            + 'than fourfold however small it started out.',
        'A file past 80 megapixels is refused rather than opened, because a browser tab cannot decode one, '
            + 'and the panel names the figure it counted.',
    ],
