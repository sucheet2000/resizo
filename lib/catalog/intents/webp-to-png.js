 ...
 * transparent areas, and JPEG would fill them with a solid colour. So the copy is built
...
const webpToPng = {
    ...
    changes: {
        ...
        doesNot: [
            'The smoothing a lossy WebP encoder already applied is copied faithfully, not cleaned up or '
                + 'reversed.',
        ...
        steps: [
            ...
            },
            {
                ...
                text: 'The picture is decoded and those exact pixels are stored in a PNG. The alpha channel travels '
                    + 'with them, so a cut-out arrives still cut out rather than sitting on a solid rectangle.',
            },
            {
            ...
            blocks: [
                ...
                },
                {
                    ...
                    text: 'Take the same file to [WebP to JPG](/webp-to-jpg) and every empty area is filled in with '
                        + 'white, or a colour you choose, because JPEG has nowhere to store it. That page is the right one when the '
    ...
    limitations: [
        'There is no batch mode on this page, so a set of stickers or logos has to be fed through one file at '
            + 'a time, each under 20 MB.',
        'A full-size lossless encode is real work, and a device without room for the decoded frame and the '
            + 'PNG together is refused before it is attempted.',
        'Nothing here writes an animated PNG, so a moving WebP arrives as its opening frame and the sequence '
            + 'stays in the original file.',
    ],
