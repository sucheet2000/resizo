...
/**
 * A non-enumerable marker the route wrapper reads to record resize_history and
 * to log the output size, without re-measuring the body it just built. Only
 * imageResponse sets it, so a zip (bulk) or a CSV (export) is never mistaken
 * for a single processed image.
 */
export const HISTORY_META = Symbol('resizo.historyMeta');
...
export function imageResponse(buffer, { format, filename, remaining, extra } = {}) {
    ...
    Object.defineProperty(response, HISTORY_META, {
