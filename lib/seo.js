...
/**
 * The builder's own profile — the account that owns GITHUB_REPO_URL, so it
 * is the one profile the repository itself confirms. Linked visibly (the
 * About page, the footer) and carried as the author's `url` in JSON-LD; no
 * other social profile is ever asserted, because none is verifiable from here.
 */
export const GITHUB_PROFILE_URL = 'https://github.com/sucheet2000';
...
/**
 * The metadata for one guide entry.
 *
 * Two things differ from an intent. The OG type is `article`, because a guide
 * is a document with an author and a date rather than a tool — the Article
 * JSON-LD on the page says the same thing, and a scraper reading only the OG
 * tags should not be told it is looking at a web app. And a guide declares no
 * ogImage of its own: there is no per-guide card art, so it takes the site
 * image rather than pointing at a file that does not exist.
 */
export function guideMetadata(guide) {
    return buildMetadata({
        title: guide.title,
        description: guide.description,
        path: guide.path ?? `/guides/${guide.slug}`,
        type: 'article',
        robots: guide.indexable ? INDEXABLE_ROBOTS : NOINDEX_ROBOTS,
    });
}
