...
import { bytesToKb, TARGET_UNREACHABLE_CODE } from '@/lib/image-client/target-bytes';
import { MAX_DIMENSION, MAX_DPI, MAX_TARGET_BYTES, MIN_DPI } from '@/lib/limits';
 ...
 */
export function recoveryFor(error, { format, isCustom = true } = {}) {
    ...
    if (code === TARGET_UNREACHABLE_CODE) {
        return [
            // PNG has no quality dial in this build, so there is nothing to lower.
            ...(chosenFormat === 'png' ? [] : ['lower-quality']),
