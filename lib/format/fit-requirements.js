...
const MESSAGES = {
    ...
    bothSides: 'Enter a width and a height.',
...
export function resolveRequirements({
...
} = {}, { defaultDpi = DEFAULT_PHYSICAL_DPI } = {}) {
    const errors = {};
    ...
    if (textOf(width) === '' && textOf(height) === '') {
        // Nothing typed on either side is one request, not two faults.
        errors.size = errors.size ?? MESSAGES.bothSides;
    } else {
    ...
    // A physical size with no DPI typed takes the caller's default — Resizo's

    // own 300 on the fitter — or is refused when the caller allows none, which

    // is the passport page's rule: its numbers come from an authority, and a

    // resolution nobody asked for must not quietly become part of them.

    let effectiveDpi = typedDpi;

    if (typedDpi === null && isPhysical && dpiText === '') {

        if (defaultDpi === null) errors.dpi = errors.dpi ?? MESSAGES.dpiNeeded;

        else effectiveDpi = defaultDpi;
