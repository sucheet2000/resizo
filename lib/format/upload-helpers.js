 ...
 *
 * NO REACT, AND THAT IS LOAD-BEARING. It used to sit in lib/hooks/, where every
 * other file opens with 'use client' and pulls in React hooks — and the WORKER
 * engine imports this module directly (lib/image-client/operations.js). The
 * worker's React-free guarantee rested on nobody ever adding a useState to a
 * file in a directory called hooks, which is a reasonable thing to do there and
 * would have dragged React into the worker bundle in silence. It lives in
 * lib/format/ so the boundary is the directory rather than a habit.
 */
import {
    MAX_BULK_FILES,
...
const FORMAT_LABELS = {
    ...
    heif: 'HEIF',
    avif: 'AVIF',
    // Not an image, and the only entry here that is not. /merge-pdf takes
    // documents, and it reads its accept list, its constraint line and its
    // picker filter through the same three tables every image tool does —
    // a second set for one tool is how the drop zone and the engine start
...
export const rejectReason = {
    ...
    wrongType: (formats) => {
        const accepted = (Array.isArray(formats) ? formats : []).filter(Boolean);
        const prose = formatProse(accepted, 'or');
        const article = /^[aeiou]/i.test(prose) ? 'an' : 'a';
        const fix = accepted.length > 1 ? 'Pick one of those formats.' : `Pick ${article} ${prose} file.`;
        return `That file is not ${article} ${prose} image. ${fix}`;
    },
    // The same refusal for the one tool whose files are not images. Saying
    // "not a PDF image" would be the sentence a shared message produces and
    // nobody's idea of English.
    wrongDocument: (formats) =>
        `That file is not a ${formatList(formats)}, so its pages cannot be read. Pick a ${formatList(formats)} file.`,
