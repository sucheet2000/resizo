 ...
 * guillotine. Each line sits one pixel OUTSIDE the photo it belongs to, which
 * is what lets it be drawn without touching a single pixel of the picture.
 */
function edgeLines(cells, { paperWidthPx, paperHeightPx }) {
    const verticals = new Set();
    const horizontals = new Set();

    for (const cell of cells) {
        verticals.add(cell.x - 1);
        verticals.add(cell.x + cell.width);
        horizontals.add(cell.y - 1);
...
export function layoutSheet({
    ...
    const cells = buildCells({
        ...
        photoWidthPx,
        photoHeightPx,
        xOffset: blockOffset(chosen.widthPx, photoWidthPx, marginPx, gapPx, chosen.columns),
        yOffset: blockOffset(chosen.heightPx, photoHeightPx, marginPx, gapPx, chosen.rows),
        gapPx,
    });
    ...
    const raw = guides === 'corners'
        ...
        : (guides === 'lines'
            ? edgeLines(cells, { paperWidthPx: chosen.widthPx, paperHeightPx: chosen.heightPx })
            : []);
    ...
    return {
        ...
        guides: {
            style: guides,
            thicknessPx: resolution >= THICK_GUIDE_DPI ? 2 : 1,
            marks,
        },
 ...
 */
export function sourceEnlargement({ keptWidth, keptHeight, photoWidthPx, photoHeightPx } = {}) {
    ...
    if (keptWidth >= photoWidthPx && keptHeight >= photoHeightPx) return null;

    return {
        from: { width: keptWidth, height: keptHeight },
        to: { width: photoWidthPx, height: photoHeightPx },
    };
}
