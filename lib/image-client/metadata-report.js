...
const PROBLEM_TEXT = 'Some text was too long to show in full.';
...
function buildText(bytes, blocks, read, problems) {
    ...
    if (items.some((item) => item.truncated)) problems.push(PROBLEM_TEXT);
...
function rawRow(group, tag, name, value) {
    const text = renderValue(value);
    const cut = text.length > MAX_RAW_CHARS;

    return {
        group, tag, name, value: cut ? text.slice(0, MAX_RAW_CHARS) : text, truncated: cut,
    };
}
...
function buildRaw(format, blocks, read, xmp, icc) {
    ...
    for (const [directory, entries] of [['IFD0', read.ifd0], ['Exif', read.exif], ['GPS', read.gps]]) {
        for (const entry of entries) {
            rows.push(rawRow(directory, hexTag(entry.tag), entry.name, entry.value));
        }
    }
