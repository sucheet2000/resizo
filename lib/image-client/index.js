/**
 * Browser Image Engine — public surface
 *
 * Everything the worker and the tool hooks are allowed to reach for. Import
 * from here, not from the individual modules: the split between decode, resize
 * and encode is an implementation detail and the native-versus-WASM routing is
 * expected to keep moving between them.
 *
 * The one thing every caller must do first is assessFile(). Nothing in this
 * engine allocates a pixel buffer before the job has been costed against the
 * device — see lib/image-client/capability.js for why that is not optional.
 *
 * Nothing here runs on the server. Every module needs createImageBitmap,
 * OffscreenCanvas, Blob or WebAssembly, and none of them is imported at module
 * scope by anything the server renders.
 */

export {
    assessFile,
    assessPdfJob,
    assessPdfMergeJob,
    assessPixels,
    deviceBudgetBytes,
    estimatePdfMergePeakBytes,
    estimatePdfPeakBytes,
    estimatePeakBytes,
    nativeDownscaleSupported,
    readDeviceProfile,
    wasmSupported,
    BYTES_PER_PIXEL,
    COMFORTABLE_ENCODE_PIXELS,
    HARD_MAX_SOURCE_PIXELS,
    MAX_TAB_BUDGET_BYTES,
    MIN_TAB_BUDGET_BYTES,
    PDF_DOCUMENT_COPIES,
    PDF_EMBED_FILE_COPIES,
    PDF_MERGE_SOURCE_COPIES,
} from '@/lib/image-client/capability';

export { decodeAndDownscale, decodeToImageData } from '@/lib/image-client/decode';

/**
 * Orientation is exported for the gate and for tests, not because a tool page
 * should ever reach for it: every decode route in this engine already returns
 * pixels the right way up, and a second call to applyOrientation would turn a
 * photo twice.
 */
export {
    applyOrientation,
    orientationSwapsAxes,
    orientedSize,
    readExifOrientation,
    EXIF_SCAN_BYTES,
    ORIENTATION_NONE,
} from '@/lib/image-client/orientation';

export {
    encodeImageData,
    formatSupportsQuality,
    normaliseFormat,
    BROWSER_OUTPUT_FORMATS,
    QUALITY_FORMATS,
    WEBP_TARGET_PASSES,
} from '@/lib/image-client/encode';

export {
    compressToTargetBytes,
    reachesTargetBytes,
    LOSSLESS,
    NATIVE,
    SEARCH,
    TARGET_CAPABLE_FORMATS,
    TARGET_FALLBACK_FORMAT,
} from '@/lib/image-client/compress-target';

export {
    flattenImageData,
    formatKeepsAlpha,
    hasTransparency,
    ALPHA_OUTPUT_FORMATS,
    FLATTEN_BACKGROUND,
} from '@/lib/image-client/flatten';

export {
    canUseNativeDownscale,
    centreCropRect,
    coverDimensions,
    fitWithin,
    resampleSize,
    resizeImageData,
    targetDimensions,
    FIT_COVER,
    RESIZE_METHOD,
} from '@/lib/image-client/resize';

export { cropImageData, resolveCropRect } from '@/lib/image-client/crop';

export {
    impossibleTargetMessage,
    parseTargetBytes,
    searchQuality,
    searchScale,
    TARGET_SEARCH_DEADLINE_MS,
} from '@/lib/image-client/target-bytes';

/**
 * The PDF side. Exporting buildPdf from here costs the other tools nothing:
 * @cantoo/pdf-lib is behind an `import()` inside pdf.js, exactly as the WASM
 * codecs are behind one inside codecs.js, so a visitor on /resize never
 * downloads a byte of it. Everything else below is pure arithmetic over
 * geometry and bytes and loads no writer at all.
 */
export {
    buildPdf,
    canEmbedWithoutDecoding,
    pageGeometry,
    parseMarginPoints,
    parsePageOrientation,
    parsePageSize,
    pixelsToPoints,
    shareBudgets,
    stripJpegMetadata,
    targetMissMessage,
    A4_POINTS,
    IMAGE_DPI,
    LETTER_POINTS,
    MAX_MARGIN_POINTS,
    MAX_PAGE_POINTS,
    PAGE_BACKGROUND,
    PAGE_ORIENTATIONS,
    PAGE_SIZES,
    PDF_JPEG_QUALITY,
    POINTS_PER_INCH,
} from '@/lib/image-client/pdf';

/**
 * The merge side. Same deal as above — it reaches the writer through pdf.js's
 * memoised `import()`, so nothing about it is downloaded by a visitor who never
 * opens /merge-pdf. It loads no codec at all: merging copies page objects and
 * never decodes a pixel.
 */
export {
    buildDefaultPlan,
    mergePdfs,
    parseMergePlan,
    MERGE_FILENAME_PREFIX,
    MERGE_PRODUCER,
} from '@/lib/image-client/pdf-merge';

export {
    runOperation,
    CLIENT_OPS,
    HEIC_JPEG_QUALITY,
    JobError,
    MULTI_FILE_OPS,
    PROGRESS,
} from '@/lib/image-client/operations';

export { fileFromFormData, optionsFromFormData, FILE_FIELD } from '@/lib/image-client/form-options';

/**
 * The worker wrapper is NOT re-exported here. index.js is the engine's surface
 * and is imported by the worker itself; pulling client.js in would make the
 * worker import a module that constructs a worker. Reach for
 * lib/image-client/client.js directly, or use lib/hooks/useLocalProcess.js.
 */

export {
    loadHeifDecoder,
    loadJpegDecoder,
    loadJpegEncoder,
    loadPngDecoder,
    loadPngEncoder,
    loadResizer,
    loadWebpDecoder,
    loadWebpEncoder,
    resetCodecs,
    WASM_BASE_PATH,
} from '@/lib/image-client/codecs';
