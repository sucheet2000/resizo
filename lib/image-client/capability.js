...
/**
 * THE ONE PLACE A CODEC'S OWN APPETITE IS PRICED, and the reason it had to
 * exist.
 *
 * Everything above prices a stage by the surfaces it holds, which is right for
 * MozJPEG, libwebp and the PNG encoder: all three cost roughly the pixels twice
 * plus a working set, so the shape of the job decides the peak and the codec
 * does not. libavif is different in kind. It builds an AV1 encoder's entire
 * working state inside a WebAssembly heap that grows with the picture and NEVER
 * SHRINKS afterwards.
 *
 * Measured 2026-09-11 (Node 20, M1 Pro, @jsquash/avif 2.1.1 single-threaded,
 * heap read as HEAPU8.byteLength after the encode):
 *
 *     0.31 MP     16 -> 19 MB      10.6 MB/MP   (init dominates)
 *     1.71 MP     16 -> 60 MB      25.9 MB/MP
 *    12.00 MP     16 -> 345 MB     27.4 MB/MP
 *    24.00 MP     16 -> 670 MB     27.2 MB/MP
 *
 * 27.4 MB per megapixel is 28.7 bytes per pixel; 30 is that rounded up, in the
 * same direction every other constant in this file rounds. It is charged ON TOP
 * of the input copies, because the JS-side ImageData is still resident while
 * the codec copies it in.
 *
 * The table is keyed only by the formats that need it. Everything absent is
 * charged nothing extra, which is what keeps every existing estimate — and
 * every existing caller, which passes no output format at all — exactly where
 * it was.
 */
export const ENCODE_BYTES_PER_PIXEL_BY_FORMAT = {
    avif: 30,
};

/** The extra heap one encode of `format` costs, per output pixel. */
function encodeHeapBytesPerPixel(format) {
    if (typeof format !== 'string') return 0;
    return ENCODE_BYTES_PER_PIXEL_BY_FORMAT[format.trim().toLowerCase()] ?? 0;
}
...
const OPERATION_PROFILES = {
    ...
    // /favicon-generator. Six resamples and six encodes off one decode, and
    // the profile is aimed at the largest of them: 512 x 512 is the only
    // surface big enough to matter, and the five smaller icons together are a
    // rounding error beside the source photograph that is already in memory.
    icons: { resizes: true, encodes: true },
...
/**
 * A 2x2 AVIF, 301 bytes, written by sharp 0.35.3 (libvips 8.18.3, libheif
 * 1.23.1, aom 3.14.1) from four raw RGB pixels on 2026-09-11:
 *
 *     sharp(Buffer.from([255,0,0, 0,255,0, 0,0,255, 255,255,0]),
 *           { raw: { width: 2, height: 2, channels: 3 } })
 *         .avif({ quality: 20, effort: 9 }).toBuffer()
 *
 * It is a real file — brand `avif`, one `av01` item, an `ispe` saying 2x2 — and
 * tests/lib/image-client/capability-avif.test.js re-parses it with the header
 * reader to prove that, so it cannot rot into a blob that every decoder
 * rejects and quietly make the probe answer "no" for everyone.
 *
 * It is INLINE rather than fetched because the answer is needed at intake,
 * before a visitor has waited for anything, and 301 bytes of base64 is cheaper
 * than a request. 404 characters, once, in the page's own chunk.
 */
const AVIF_PROBE_BASE64 = '';

/**
 * The memoised PROMISE, not the answer — two intakes in one tick share it —
 * KEYED ON THE DECODER IT PROBED.
 *
 * A browser does not swap its image decoder mid-session, so in production the
 * key never changes and this is a plain memo. It is keyed anyway because the
 * answer is a statement ABOUT `createImageBitmap`, and a cached statement about
 * a function that is no longer the one installed is not an answer, it is a
 * stale one. That distinction is invisible in a tab and load-bearing in the
 * suite, where three test files install decoders of their own: without the key,
 * whether an unreadable AVIF is reported as a damaged FILE or an old BROWSER
 * would depend on the order the tests happened to run in.
 */
let avifDecodeProbe = null;

/** Test-only: makes the next canDecodeAvif() run the probe again. */
export function resetAvifDecodeProbe() {
    avifDecodeProbe = null;
}

/**
 * Whether this scope has an image decoder at all.
 *
 * Separate from canDecodeAvif because the two answers differ: this one is
 * synchronous and certain, and a `false` here means AVIF is out of reach
 * without anything needing to be decoded to find out.
 */
export function nativeDecodeSupported() {
    return typeof globalScope().createImageBitmap === 'function';
}
...
function decodeBase64(text) {
    ...
    // Node, where the engine tests run. Buffer is a Uint8Array subclass.
    return Uint8Array.from(scope.Buffer.from(text, 'base64'));
}

/**
 * Whether THIS browser can open an AVIF at all.
 *
 * `nativeDownscaleSupported()` cannot answer this: it is a capability probe,
 * not a format probe, and it keeps returning true on a Safari 16.0 that has
 * `createImageBitmap` and no AVIF decoder behind it. The only way to find out
 * is to try, so this tries — once per session, on 301 bytes.
 *
 * There is no WASM decoder behind a false answer and there is deliberately not
 * going to be one: the package's own decoder measured 3.4-4.9x slower than
 * libvips at 13.9 MB of heap per megapixel, for 267 KB brotli. A browser that
 * cannot read AVIF is told so in a sentence naming the versions that can.
 *
 * A rejection, a missing global and a 0x0 bitmap are all "no". The last one
 * matters: a decoder that parses the container and hands back a blank canvas
 * has not decoded anything, and libheif does exactly that with AV1 payloads.
 *
 * @returns {Promise<boolean>}
 */
export function canDecodeAvif() {
    const scope = globalScope();
    const decoder = scope.createImageBitmap;

    if (avifDecodeProbe && avifDecodeProbe.decoder === decoder) return avifDecodeProbe.answer;

    const answer = (async () => {
        if (typeof decoder !== 'function' || typeof scope.Blob !== 'function') return false;

        let bitmap = null;
        try {
            const blob = new scope.Blob([decodeBase64(AVIF_PROBE_BASE64)], { type: 'image/avif' });
            bitmap = await decoder(blob);
            return bitmap.width === 2 && bitmap.height === 2;
        } catch {
            // An engine without AVIF is an expected branch, not an incident.
            return false;
        } finally {
            bitmap?.close?.();
        }
    ...
    avifDecodeProbe = { decoder, answer };
    return answer;
}
 ...
 * @param {string} [input.outputFormat]  what is being WRITTEN, when the encoder
 *   for it costs more than the surfaces it is handed. Absent means "charge what
 *   this model always charged", which is the honest answer for every format
 *   except AVIF — see ENCODE_BYTES_PER_PIXEL_BY_FORMAT.
 */
export function estimatePeakBytes({
    ...
    // The codec's own heap, on top of the surfaces, for the one encoder that
    // has one. Priced against the OUTPUT picture, because that is what the
    // encoder is handed — a 12 MP photo resized to 1000 px wide costs an AVIF
    // encode of 1000 px, not of 12 megapixels.
    const outputPixels = (targetWidth ?? sourceWidth) * (targetHeight ?? sourceHeight);
    const encodeHeap = profile.encodes
        ? outputPixels * encodeHeapBytesPerPixel(outputFormat)
        : 0;

    const encodeStage = profile.encodes
        ? destination * (ENCODE_INPUT_COPIES + ENCODE_WORKING_SET_FACTOR) + encodeHeap
        : 0;
...
export const OUTPUT_EXPANSION_BY_FORMAT = {
    ...
    png: 9,
    webp: 1.3,
    // Measured under 1 on the bench photo — q80 AVIF is 56,316 bytes against
    // the 84,376-byte JPEG it was made from — so 1.3 is deliberately
    // conservative, matching WebP rather than claiming the saving in advance.
    // No bulk tool writes AVIF today; this is here so the factor is right the
    // moment one does, rather than falling to DEFAULT_OUTPUT_EXPANSION's 9x and
    // refusing every batch.
    avif: 1.3,
};
 ...
 * @param {string} [options.outputFormat]  what will be WRITTEN, for the one
 *   encoder whose cost is not a multiple of the surface it is handed
