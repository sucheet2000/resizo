...
/**
 * What the single most expensive file in the batch costs while it is the one
 * being worked on. The same split estimatePdfPeakBytes makes, for the same
 * reason: processBatch awaits each file before starting the next, so this is the
 * LARGEST single file's stage and never the sum. Charging twenty decode surfaces
 * at once would refuse every batch the tool exists for.
 *
 ...
 * size. The per-file gate (assessJob, run inside lib/upload/process-file.js)
 * re-costs it properly the moment the dimensions are known.
 */
function largestFileStageBytes(files, operation, nativeDownscale) {
    let largest = 0;
    ...
    for (const file of files) {
        ...
        if (stage > largest) largest = stage;
    }
...
export function estimateBatchPeakBytes({
    ...
    return Math.round(archiveBytes + largestFileStageBytes(list, operation, nativeDownscale));
}
 ...
 *
 * Returns 0 when the largest file alone already fills the budget — the batch
 * cannot hold anything, which is a true answer and one the caller must handle
 * rather than a floor invented to avoid it.
 */
export function batchArchiveBudgetBytes({
    ...
    const list = Array.isArray(files) ? files : [];
    const spare = deviceBudgetBytes(device) - largestFileStageBytes(list, operation, device.nativeDownscale);
