...
/**
 * @cantoo/pdf-lib is around a megabyte of JavaScript. Four of the five tools on
...
export function loadPdfWriter() {
    if (!writerPromise) {
        writerPromise = import('@cantoo/pdf-lib')
 ...
 *
 * Walks the segment table only, and stops at SOS: everything from the start of
 * scan to the end of the file is entropy-coded data and is copied in one piece,
 * so no run of compressed bytes can be mistaken for a marker. Nothing is
 * decoded, nothing is re-encoded, and the compressed picture is bit-identical.
 *
 * Returns the SAME array when there was nothing to drop, so a JPEG that carries
 * no metadata — which is every file this engine has already written — costs
 * nothing at all here.
 *
 * A file that does not parse is handed back untouched. The alternative is
 * refusing a JPEG that a decoder might still have been able to read, and the
 * embedder is about to parse the same header anyway and will say so properly.
 *
...
export function stripJpegMetadata(bytes) {
    ...
    while (at + 4 <= bytes.length) {
        ...
        // measure and nothing after them worth walking.
    ...
    // The tail — the start of scan and every compressed byte after it — in one
    // slice. This is where the picture actually lives and it is never touched.
    keep.push({ start: at, end: bytes.length });

    const stripped = new Uint8Array(bytes.length - dropped);
    let offset = 0;
