...
import { referenceTickRange } from '@/lib/format/print-sheet';
...
/** Cut guides on a page: half a point of grey, and a full point of black for
 * the measuring line. Points rather than pixels, because a PDF has no pixels —
 * a hairline at 0.5 pt prints as one printer dot on any device. */
const SHEET_GUIDE_POINTS = 0.5;
const SHEET_REFERENCE_POINTS = 1;
...
export async function buildSheetPdf({ photoJpegBytes, layout } = {}) {
    ...
    for (const mark of layout.guides.marks) {
        page.drawLine({
            start: { x: toPoints(mark.x1), y: fromTop(mark.y1) },
            end: { x: toPoints(mark.x2), y: fromTop(mark.y2) },
            thickness: SHEET_GUIDE_POINTS,
            color: grey,
        });
    }
    ...
    if (line) {
        ...
        page.drawLine({
            start: { x: toPoints(line.x1), y: fromTop(line.y1) },
            end: { x: toPoints(line.x2), y: fromTop(line.y2) },
            thickness: SHEET_REFERENCE_POINTS,
            color: black,
        });

        for (const x of [line.x1, line.x2]) {
            page.drawLine({
                start: { x: toPoints(x), y: fromTop(tick.top) },
                end: { x: toPoints(x), y: fromTop(tick.bottom) },
                thickness: SHEET_REFERENCE_POINTS,
                color: black,
            });
        }
