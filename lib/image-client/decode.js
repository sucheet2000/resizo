...
import { readAvifHeader, AVIF_ANIMATED_MESSAGE, AVIF_DAMAGED_MESSAGE, AVIF_HEADER_SCAN_BYTES } from '@/lib/image-client/avif';
import { assessPixels, canDecodeAvif, nativeDecodeSupported, nativeDownscaleSupported, DECODE_OPERATION } from '@/lib/image-client/capability';
import {
    loadHeifDecoder,
    loadJpegDecoder,
    loadPngDecoder,
...
/** The first `count` bytes, without pulling a 20 MB file into memory. */
async function readPrefix(source, count) {
    if (isBlobLike(source)) {
        return new Uint8Array(await source.slice(0, count).arrayBuffer());
    }
    const bytes = toUint8(source);
    if (!bytes) throw new Error('No valid file was provided.');
    return bytes.subarray(0, Math.min(bytes.length, count));
}

/* ----------------------------------------------------------------- AVIF */

/**
 * A refusal shaped so operations.js can rebuild it as a JobError.
 *
 * Same form as gateRefusal and damagedFileRefusal below and for the same
 * reason: JobError lives in operations.js, operations.js imports this module,
 * and importing it back would close a cycle.
 */
function avifRefusal(code, message, suggestion) {
    return Object.assign(new Error(message), { code, suggestion });
}

const AVIF_ANIMATED = () => avifRefusal(
    'avif-animated',
    AVIF_ANIMATED_MESSAGE,
    'Save a single frame as a JPEG or PNG first, then bring that here.',
);

const AVIF_DAMAGED = () => avifRefusal(
    'avif-damaged',
    AVIF_DAMAGED_MESSAGE,
    'Choose the original file again, or try a different image.',
);

/**
 * THE VERSIONS ARE IN THE MESSAGE, NOT IN THE SUGGESTION, and that is a
 * decision about what a visitor actually reads.
 *
 * A gate refusal reaches the panel through `refusalMessage()`, which joins the
 * reason and the suggestion into one string. A JobError does not: the tool
 * panels render `message`, and only /passport-photo's recovery block renders
 * `suggestion` as well. So a second half left in `suggestion` would be invisible
 * on /convert and /resize — which are the only two pages an AVIF can reach.
 *
 * "Chrome 85, Firefox 93, and Safari 16 ... can" is the half that makes this
 * refusal actionable rather than a dead end, so it is in the sentence that is
 * guaranteed to be shown. There is no separate suggestion because the message
 * already says the only thing there is to do.
 */
const AVIF_UNSUPPORTED_BROWSER = () => avifRefusal(
    'avif-unsupported-browser',
    'This browser cannot open AVIF images. Chrome 85, Firefox 93, and Safari 16 on iOS 16 or macOS Ventura and later can.',
    null,
);

/**
 * Everything that has to be true about an AVIF BEFORE a decoder is asked for it.
 *
 * THE ANIMATION CHECK IS THE REASON THIS EXISTS. `createImageBitmap` on an
 * animated AVIF succeeds in every engine and hands back the first frame, so
 * without this a three-second clip would come out as a still JPEG and the
 * visitor would be told it worked. The brand is readable in the first sixteen
 * bytes; the refusal costs one slice and no decode.
 *
 * Returns the header, so the caller can report the source's bit depth without
 * reading the file twice.
 */
async function guardAvif(source) {
    const header = readAvifHeader(await readPrefix(source, AVIF_HEADER_SCAN_BYTES));

    // A container with no readable size (every AVIF image item must carry an
    // ispe) is refused here rather than handed to a decoder that will fail
    // with a sentence about "an image".
    if (!header || header.width === null || header.height === null) throw AVIF_DAMAGED();
    if (header.animated) throw AVIF_ANIMATED();

    return header;
}

/**
 * Why an AVIF that would not decode did not decode.
 *
 * The two answers need different words and different actions — "this file is
 * broken" against "this browser is too old" — and the only way to tell them
 * apart is to ask the browser to decode something that is definitely fine. The
 * probe is 301 bytes and runs once per session.
 */
async function avifDecodeFailure() {
    // A scope with no image decoder at all needs nothing decoded to answer.
    // Checked first so the certain case never depends on a probe.
    if (!nativeDecodeSupported()) return AVIF_UNSUPPORTED_BROWSER();

    return (await canDecodeAvif()) ? AVIF_DAMAGED() : AVIF_UNSUPPORTED_BROWSER();
}
...
/**
 * THREE FULL SURFACES USED TO BE LIVE HERE; NOW TWO ARE.
 *
 * capability.js charges DECODE_SURFACE_COPIES = 2 — "the codec's own output
 * and the ImageData copy handed back to JS". On this path there was a third it
 * never counted: the OffscreenCanvas backing store, which
 * `willReadFrequently: true` explicitly forces onto the CPU side. The callers
 * close the bitmap in a `finally`, i.e. after this function has already
 * returned, so at the getImageData line the tab held bitmap + canvas +
 * ImageData at once. On a 24 MP photo that is 288 MB where the gate was told
 * 192 MB — and this is the DEFAULT lane for every image tool on every browser
 * with createImageBitmap.
 *
 * Closing between the draw and the read makes the documented count TRUE, which
 * is strictly better than raising the estimate to 3: it lowers what the tab
 * actually allocates rather than refusing more jobs. close() is idempotent, so
 * the callers' `finally` blocks stay correct.
 *
 * The dimensions are captured FIRST because close() sets width and height to
 * zero. Reading them off the bitmap after closing would hand back a 0x0 image,
 * and a test that only checked pixel values would not notice.
 */
...
/**
 * What a person is told when a decoder cannot get through the file.
 *
 * A half-downloaded photo and a failed AirDrop are the two most common broken
 * files a public tool receives, and until this existed the second of them
 * produced `Program terminated with exit(1)` — MozJPEG calling emscripten's
 * `exit()`, whose ExitStatus is a PLAIN OBJECT rather than an Error. It is
 * short and single-line, so the worker's sanitiser judged it readable and
 * showed it to the visitor verbatim. @jsquash/png has the same shape of hole
 * with a different string: a WebAssembly trap escapes as `unreachable`.
 *
 * Neither string means anything to the person holding the phone, and neither
 * can be acted on. Both are now translated here, at the one place every WASM
 * decode goes through.
 */
const DAMAGED_FILE_MESSAGE = 'This image file appears to be damaged and could not be read.';
const DAMAGED_FILE_SUGGESTION = 'Choose the original file again, or try a different image.';

/**
 * The refusal for a decoder that could not read the bytes.
 *
 * Wears a `code` and a `suggestion` for the same reason gateRefusal does —
 * operations.js rebuilds it as a JobError and cannot be imported back from
 * here — and carries the original throwable as `cause` so the emscripten or
 * WebAssembly detail is still there in a console for anyone debugging. Only
 * the sentence crosses the worker boundary; `cause` never does.
 */
function damagedFileRefusal(cause) {
    return Object.assign(new Error(DAMAGED_FILE_MESSAGE), {
        code: 'invalid-file',
        suggestion: DAMAGED_FILE_SUGGESTION,
        cause,
    });
}
 ...
 */
/**
 * ONE DECODER FOR THE MODULE, AND THAT IS A LEAK FIX, NOT A MICRO-OPTIMISATION.
 *
 * `HeifDecoder.prototype` in libheif-js 1.19.x has exactly two members —
 * `constructor` and `decode` — and no `free`. The context is allocated inside
 * `decode()` and `heif_context_free` is called in exactly one place: the top of
 * a SUBSEQUENT `decode()` on the SAME instance. A decoder built per call is
 * therefore a decoder whose context can never be released, and
 * `heif_context_read_from_memory` is the COPYING variant — measured, 4.96 MB of
 * unreachable WASM heap for a 4.97 MB photo, every time.
 *
 * That is the whole tool: someone converting a camera roll one photo at a time
 * spends ~1x each file, permanently. Twenty 4 MB photos is 80 MB the tab can
 * never get back, while WASM_BASELINE_BYTES in capability.js stays a flat 24 MB
 * constant — so the gate keeps approving jobs against a budget already spent,
 * and on iOS the tab is killed with no exception and no error event.
 * /jpg-to-pdf is worse: up to 20 HEIC pages leak 20 contexts in one job.
 *
 * Reuse is what libheif's own API expects. It is safe here because the engine
 * runs strictly one job at a time — client.js holds a single `active` slot and
 * queues the rest — and buildPdf decodes its pages sequentially, so this is
 * never re-entered.
 *
 * The cache is keyed to the module it was built from rather than being a bare
 * singleton. loadHeifDecoder memoises, so in the browser this is one decoder
 * for the life of the worker either way — but a decoder outliving the module
 * that created it would be a use-after-free waiting to happen, and tying the
 * two together says so in code instead of relying on a comment.
 */
let heifDecoder = null;
let heifDecoderModule = null;

function decoderFor(libheif) {
    if (heifDecoder === null || heifDecoderModule !== libheif) {
        heifDecoder = new libheif.HeifDecoder();
        heifDecoderModule = libheif;
    }
    return heifDecoder;
}
...
async function decodeHeic(source) {
    ...
    const decoder = decoderFor(libheif);
...
export async function decodeToImageData(source, { mimeOrSniff = null } = {}) {
    ...
    // Read before anything decodes: an animated AVIF would otherwise come back
    // as its first frame, and a damaged one as a browser-support problem.
    const avifHeader = format === 'avif' ? await guardAvif(source) : null;
    const sourceBitDepth = avifHeader?.bitDepth ?? null;
    ...
    if (nativeDownscaleSupported()) {
        ...
        try {
            ...
            return { data, width: data.width, height: data.height, format, viaNative: true, sourceBitDepth };
        } catch {
    ...
    if (!loadDecoder) {
        // AVIF has no WASM decoder here and never will: the browser's own is
        // 4x faster and free. So the refusal says which of the two things went
        // wrong instead of blaming the browser for a broken file.
        if (format === 'avif') throw await avifDecodeFailure();
    ...
    let decoded;
    try {
        // The codecs want a standalone ArrayBuffer, and a Uint8Array view over
        // a larger buffer would hand them the whole thing.
        decoded = await decode(bytes.buffer.slice(bytes.byteOffset, bytes.byteOffset + bytes.byteLength));
    } catch (error) {
        // Every WASM decode in the engine passes through this one call, which
        // is why the translation lives here and nowhere else. A codec that
        // cannot read the bytes has told us one thing — the file is not
        // readable — and that is what the visitor is told.
        throw damagedFileRefusal(error);
    }
    ...
    return { data, width: data.width, height: data.height, format, viaNative: false, sourceBitDepth };
}
...
export async function decodeAndDownscale(source, maxWidth, {
    ...
    // OUTSIDE the try below, deliberately. The catch there falls back to a full
    // decode on any native failure, and an animated or damaged AVIF must be
    // refused rather than retried — a first frame is not a conversion.
    const avifHeader = format === 'avif' ? await guardAvif(source) : null;
    const sourceBitDepth = avifHeader?.bitDepth ?? null;
    ...
    try {
        if (sourceWidth && sourceHeight) {
            ...
            // before the resize.
            // `exact` means the caller already decided the box and this must
            // not second-guess it. Re-deriving with fitWithin here is what made
            // the lane unusable: the caller's target rounds each side on its
            // own, fitWithin rounds both from one shared ratio, and the two
            // disagree by a pixel often enough that ~21% of percentage resizes
            // were pushed onto the 1.5 s WASM resampler to avoid a mismatch the
            // caller never asked about.
            const fit = exact
                ? {
                    width: maxWidth,
                    height: maxHeight,
                    changed: maxWidth !== sourceWidth || maxHeight !== sourceHeight,
                }
                : fitWithin(sourceWidth, sourceHeight, maxWidth, maxHeight);
