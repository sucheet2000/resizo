...
function parseFormat(raw) {
    ...
    if (!value || !ALLOWED_OUTPUT_FORMATS.includes(value)) {
        return fail(INVALID_FORMAT, `Invalid output format. Must be ${joinOr(ALLOWED_OUTPUT_FORMATS)}.`);
    }
...
/**
 * IHDR is fixed at offset 8 and is always the first chunk, so a PNG needs no
 * walk. Colour types 4 and 6 are the two that carry an alpha channel; 3 is a
 * palette, whose transparency lives in a tRNS chunk further in, and this engine
 * never writes one — @jsquash/png emits full-colour RGBA.
 */
