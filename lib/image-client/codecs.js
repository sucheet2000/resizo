...
export function loadAvifEncoder() {
    return once('avif:encode', async () => {
        // The single-threaded emscripten glue, imported directly. The package's
        // own encode.js chooses at run time between this build and the
        // threaded one, and the threaded glue spawns a nested worker from
        // import.meta.url; a bundler that follows that graph never finishes —
        // the first production build carrying this package hung in
        // compilation until this import was narrowed. Only avif_enc.wasm is
        // shipped, so the threaded path could never have run anyway.
        const [{ default: factory }, { defaultOptions }] = await Promise.all([
        ...
        const instance = await factory({ noInitialRun: true, locateFile });

        // What the package's wrapper does around the glue, minus its bit-depth
        // and lossless branches: this engine hands over 8-bit RGBA and never
        // asks for lossless.
        const encode = async (imageData, options = {}) => {
            const merged = { ...defaultOptions, ...options };
            const output = instance.encode(
