...
const FIELD_MAPS = {
    resize: { width: 'width', height: 'height', scale: 'scale', format: 'format' },
    crop: { x: 'crop_x', y: 'crop_y', width: 'crop_width', height: 'crop_height' },
    // `output_format` has no counterpart on app/api/compress/route.js, which
    // always answers in the source format. It is the browser build's one added
    // field, and it exists because PNG cannot reach a byte target here without
    // shrinking the picture — see lib/image-client/compress-target.js. The
    // server ignores it, so posting it costs the fallback lane nothing.
    compress: { quality: 'quality', targetBytes: 'targetBytes', format: 'output_format' },
