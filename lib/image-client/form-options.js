...
const FIELD_MAPS = {
    ...
    // `quality` is the bulk converter's; the single-file page sends none and
    // gets DEFAULT_QUALITY, exactly as it always did.
    convert: { format: 'target_format', quality: 'quality', background: 'background' },
    ...
    // /passport-photo-print: one physical layout, described in millimetres.
    // `fit` here is the fitter's cover/contain question and NOT signature's
    // three-valued `fit`, which is why the two never share a field name in one
    // form; the four crop_ fields are the same rectangle in the same source
    // pixels every other tool posts under those names.
    sheet: {
    ...
    // /favicon-generator: which way a mark that is not square reaches a square,
    // the frame it was cropped with (cover only) and what sits behind it —
    // 'transparent' as well as the colours, which is this op's own addition to
    // the background field every other tool posts under that name.
    icons: {
        geometry: 'geometry',
        x: 'crop_x',
        y: 'crop_y',
