...
/**
 * The one import here, and it is deliberate. capability.js is already in the
 * main-thread graph (useLocalProcess imports assessJob from it), so this costs
 * no bytes — and the page is the ONLY place that can read the device
 * correctly: `maxTouchPoints` exists on Navigator and not on WorkerNavigator,
 * so a worker re-deriving the profile reads an iPad as a Mac and hands itself a
 * 1024 MiB budget where the real ceiling is 614 MiB.
 */
import { readDeviceProfile } from '@/lib/image-client/capability';
...
function handleMessage(event) {
    ...
    switch (message.type) {
        case 'progress':
            ...
            break;
        case 'done':
            // A worker just finished a job, so whatever went wrong earlier was
            // not the environment refusing to run one. Without this the counter
            // measured "two failures EVER" rather than the two CONSECUTIVE
            // failures handleWorkerFailure's own comment describes — so one
            // dropped chunk fetch early on and another an hour later latched
            // the tab onto the main thread for the rest of the session.
            workerFailures = 0;
            resolveCaller(job, message.result);
            // BEFORE release(), which pumps the queue: the next job must build
            // its own worker rather than inherit a 345 MB heap from this one.
            recycleAfterLargeAvifEncode(message.result);
            release(job);
            break;
        case 'error':
...
/**
 * Throws the worker away when the job it just finished left an AVIF heap too
 * big to carry into the next one. See AVIF_WORKER_RECYCLE_HEAP_BYTES.
 *
 * Only ever reached on the worker lane. The main-thread fallback has nowhere to
 * put the heap but the page itself, and terminating the page is not on offer.
 */
function recycleAfterLargeAvifEncode(result) {
    if (result?.format !== 'avif') return;

    const heap = Number(result.avifHeapBytes);
    if (!Number.isFinite(heap) || heap <= AVIF_WORKER_RECYCLE_HEAP_BYTES) return;
...
/**
 * The WebAssembly heap an AVIF encode may leave behind before the worker is
 * thrown away and rebuilt.
 *
 * libavif's heap NEVER SHRINKS. Measured 2026-09-11 on @jsquash/avif 2.1.1:
 * 16 MB after init, 60 MB after one 1.7 MP encode, 345 MB after a 12 MP one,
 * 670 MB at 24 MP — and twenty encodes in a row stayed exactly where the first
 * one left it. Nothing gives it back: not finishing the job, not dropping the
 * ImageData, not the next file. So a worker that wrote one large AVIF is a
 * worker carrying that heap into every job after it, against a tab budget that
 * can be as little as 192 MB on a phone.
 *
 * 96 MB sits above what an ordinary photograph leaves (60 MB at 1.7 MP) and far
 * below a large one (345 MB at 12 MP), so the common case pays nothing and the
 * expensive case cannot accumulate. Rebuilding costs a worker construction and
 * a codec fetch that comes straight out of the HTTP cache.
 */
export const AVIF_WORKER_RECYCLE_HEAP_BYTES = 96 * 1024 * 1024;
...
async function runOnMainThread(job) {
    try {
        ...
        const result = await runOperation(job.op, job.source, job.options, {
            ...
            // The fallback lane runs the same gates, so it gets the same
            // profile. It happens to be on the page already, but passing it
            // explicitly keeps the two lanes provably identical.
        ...
        instance.postMessage(
            {
                ...
                source: job.source,
                options: job.options,
                // Read here, on the page, because the worker cannot read it
                // correctly. See the import note at the top of this file.
                device: readDeviceProfile(),
            },
