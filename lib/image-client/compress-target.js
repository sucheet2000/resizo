...
async function attemptAtSize({
    ...
    // WEBP GIVES UP ITS NATIVE LANE HERE, AND ONLY HERE.
    //
    // Under the keep policy libwebp's rate controller is the right first move —
    // one encode instead of eight, 47.8 s against 66.6 s over 84 image/target
    // pairs — and compressToTargetBytes still opens with it. But it reaches a
    // byte count by picking a quality ITSELF, in float, with no lower bound:
    // @jsquash/webp exposes no qmin or qmax to give it one. A WebP asked for
    // 20 KB through that lane can come back at an effective quality in the
    // teens, which is exactly the trade minQuality exists to refuse.
    //
    // Under fit the floor is the policy's whole promise, and it has a second
    // lever the keep policy does not: it can make the picture smaller instead.
    // So WebP pays one extra encode here and is probed exactly as JPEG is.
...
export async function fitUnderTargetBytes({
     ...
     *
     * Every step pays exactly one floor probe, and exactly ONE step can ever
     * pay the quality search, because a floor that fits ends the walk. So the
     * ceiling is every step's floor plus one search: 17 encodes for JPEG and
     * WebP at the shipped constants, where the un-probed version's ceiling was
     * 72. WebP counts the same as JPEG now that it has no native attempt on
     * this lane.
     *
     * PNG COUNTS DIFFERENTLY, and it has to. It has no quality axis in this
     * build, so it never reaches the search and its real ceiling is one encode
     * per step. Charging it for a search it cannot run leaves the bar at 9 of
     * 17 — 53% — on a job that ran to completion, and a bar that stops there
     * reads as a hang on the one tool with no server to blame it on.
     */
    const totalProbes = (maxSteps + 1) + (reachesTargetBytes(format) ? maxIterations : 0);
