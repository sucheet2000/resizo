...
async function handleRun({ jobId, op, source, options, device }) {
    ...
    try {
        ...
        const result = await runOperation(op, source, options ?? {}, {
            ...
            checkCancelled,
            // Sent by the page. WorkerNavigator has no maxTouchPoints, so this
            // thread cannot tell an iPad from a Mac and must not try.
