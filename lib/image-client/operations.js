...
import { runDpi } from '@/lib/image-client/dpi';
 ...
 *
 * WHITE, NOT BLACK. Everywhere else a transparent pixel is composited onto
 * black, because that is what libvips does with no background given and what
 * /png-to-jpg's copy promises. A signature is going onto a form, and a black
 * box around the ink is not a defensible default for that. It is still only a
 * default — the field is honoured when it is set.
 */
...
/**
 * The rectangle to keep. All four values absent means the whole image, because
 * a signature that already fills its scan needs no crop; ANY of the four
 * present means all four must be, which is parseCropParams' own rule and the
 * reason a half-filled rectangle is a refusal rather than a guess.
 */
function resolveSignatureCrop(options, { sourceWidth, sourceHeight }) {
...
async function runSignature(context) {
    ...
    const crop = resolveSignatureCrop(options, {
        sourceWidth: decoded.width,
        sourceHeight: decoded.height,
    });
