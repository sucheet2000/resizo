...
async function runHeic(context) {
    ...
    // The output is always JPEG, which has no alpha. A camera HEIC has no alpha
    // either, so in practice this is the same-object return — but libheif hands
    // back interleaved RGBA, and a HEIC carrying an alpha auxiliary channel
    // would otherwise reach MozJPEG with the byte still live, exactly as
    // /resize did. Consistency with runConvert is the point.
    const pixels = flattenImageData(decoded.data, parseBackground(context.options?.background));
    const encoded = await encodeStage(context, pixels, { format: 'jpeg', quality: HEIC_JPEG_QUALITY });
