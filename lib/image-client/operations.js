...
import { ICON_ASSETS, ICO_SIZES, iconEnlargedFrom } from '@/lib/format/icon-package';
...
import { BROWSER_OUTPUT_FORMATS, encodeImageData, formatSupportsQuality, normaliseFormat } from '@/lib/image-client/encode';
import { BACKGROUND_PRESETS, flattenImageData, formatKeepsAlpha, parseBackground } from '@/lib/image-client/flatten';
...
/**
 * What this job is going to WRITE, for the memory gate.
 *
 * The gate prices a job by pixels and stage shape and never by codec, with ONE
 * exception: libavif builds its whole encoder state in a WebAssembly heap that
 * grows with the picture and never shrinks — 27.4 MB per megapixel, so a 12 MP
 * AVIF encode needs 345 MB the model would otherwise never have been told
 * about. capability.js keys that off the output format, so the format has to
 * reach it.
 *
 * Passed through raw rather than resolved per op. The sentinels ('original',
 * 'same', absent) are not real formats and capability.js ignores anything it
 * has no figure for, so an op that writes JPEG is quoted exactly what it was
 * quoted before — and no op that honours a sentinel can resolve one to AVIF,
 * because /compress is the only one that does and it cannot write AVIF at all.
 */
function costedOutputFormat(options) {
    return normaliseFormat(options?.format);
}
...
async function decodePixels(context, target) {
    ...
    if (!sourceWidth || !sourceHeight) {
        ...
        const recheck = assessPixels({
            ...
            operation,
            outputFormat: costedOutputFormat(context.options),
            device,
...
/**
 * THE ENCODER'S WORD IS NOT EVIDENCE FOR ITSELF.
 *
 * Every other format this engine writes has a decoder in the same browser that
 * will be opening the download minutes later, and a JPEG that came out wrong
 * fails visibly. An AVIF does not: it is a container a person's phone may or
 * may not open, written by a codec this build loaded for the first time ten
 * seconds ago. So the finished bytes are checked twice before the job says it
 * worked, by two things that share no code with the encoder:
 *
 *   THE HEADER, parsed by lib/image-client/avif.js — the brand really is
 *   `avif`, the `ispe` really says the size the pixels were, and the alpha item
 *   is there if and only if the picture had transparency to keep.
 *
 *   THE BROWSER, asked to decode what was just written. This is the check that
 *   catches a file which parses and does not display, and it is the same
 *   decoder the visitor's own machine will use.
 *
 * A row with `ok: null` is one nobody could check — a worker with no
 * `createImageBitmap` cannot decode anything — and it is NOT a failure. Same
 * convention as lib/image-client/requirements.js: `verified` is "no row said
 * no", never "every row said yes".
 *
 * @returns {Promise<{ checks: Array<{key, required, actual, ok}>, verified: boolean }>}
 */
async function verifyAvifOutput(blob, { width, height, transparent }) {
    const bytes = new Uint8Array(await blob.arrayBuffer());
    const header = readAvifHeader(bytes);

    const size = header && header.width !== null
        ? `${header.width} × ${header.height}`
        : 'unreadable';

    const decoded = await decodeBackNatively(blob);

    const checks = [
        {
            key: 'format',
            required: 'avif',
            actual: header?.brand ?? 'unreadable',
            ok: header?.brand === 'avif',
        },
        {
            key: 'dimensions',
            required: `${width} × ${height}`,
            actual: size,
            ok: header?.width === width && header?.height === height,
        },
        {
            key: 'alpha',
            required: transparent ? 'kept' : 'none',
            actual: header?.alpha ? 'kept' : 'none',
            ok: Boolean(header?.alpha) === Boolean(transparent),
        },
        {
            key: 'decodes',
            required: `${width} × ${height}`,
            actual: decoded.checked ? decoded.size : 'no decoder in this browser',
            ok: decoded.checked ? (decoded.width === width && decoded.height === height) : null,
        },
    ];

    return { checks, verified: checks.every((check) => check.ok !== false) };
}

/**
 * Opens the bytes with the browser's own decoder, if there is one here.
 *
 * `checked: false` is an honest "nobody asked" rather than a pass — a worker
 * without `createImageBitmap` exists (older Safari, and every Node test that
 * does not install one) and treating its silence as agreement would make the
 * strongest of the four checks vanish without anyone noticing.
 */
async function decodeBackNatively(blob) {
    const scope = typeof globalThis === 'undefined' ? {} : globalThis;
    if (typeof scope.createImageBitmap !== 'function') return { checked: false };
    ...
    try {
        ...
        return { checked: true, width: null, height: null, size: 'would not open' };
    } finally {
        bitmap?.close?.();
    }
}
...
/**
 * The AVIF encode, with the two checks above around it.
 *
 * Hands back the encode result plus its `checks` and `verified`, or throws. The
 * sentence names Resizo rather than libavif because a codec's name is not
 * something a person can act on, and the code is what the panel branches on.
 */
async function encodeAvifVerified(context, pixels, { quality, width, height, transparent }) {
    const encoded = await encodeStage(context, pixels, { format: 'avif', quality });
    const { checks, verified } = await verifyAvifOutput(encoded.blob, { width, height, transparent });

    if (!verified) {
        throw new JobError('Resizo couldn’t encode this image as AVIF.', {
            code: 'avif-encode-failed',
            suggestion: 'Try JPEG, PNG or WebP instead, which every browser can write here.',
        });
    }

    return { ...encoded, checks, verified };
}
...
async function runResize(context) {
    ...
    if (resolved.target && sourceWidth && sourceHeight) {
        ...
        const gate = assessPixels({
            ...
            operation: 'resize',
            // The RESOLVED format, not the requested one: /resize falls back to
            // JPEG for anything it does not recognise, and costing the request
            // would price a job that is not the one about to run.
            outputFormat: format,
    ...
    const encoded = format === 'avif'
        ? await encodeAvifVerified(context, pixels, {
            quality: DEFAULT_QUALITY,
            width: decoded.width,
            height: decoded.height,
            transparent: hasTransparentPixel(pixels),
        })
        : await encodeStage(context, pixels, { format, quality: DEFAULT_QUALITY });
    ...
    return {
        ...
        sourceBitDepth: decoded.sourceBitDepth ?? null,
    };
}
 ...
 *
 * COMPRESS_OUTPUT_FORMATS, not ALLOWED_OUTPUT_FORMATS, and the gap between them
 * is AVIF. The build can write one; this tool cannot offer one. A byte target
 * here means up to eight encodes inside TARGET_SEARCH_DEADLINE_MS, and libavif
 * exposes nothing like libwebp's `target_size` to reach a size in a single
 * pass — eight encodes of a 12 MP photo measured about eight seconds on a
 * laptop and minutes on a phone. The search would also be CORRECT if it ran:
 * the 2026-09-11 bench counted zero quality inversions over 20 steps on both
 * sources. It just would not finish, which is why this is a deferral and not a
 * capability claim.
 */
function resolveCompressFormat(requested, sourceFormat) {
    ...
    return COMPRESS_OUTPUT_FORMATS.includes(normalised) ? normalised : null;
}
...
async function runCompress(context) {
    ...
    if (!format) {
        throw new JobError(`Invalid output format. Must be ${joinOr(COMPRESS_OUTPUT_FORMATS)}.`, {
            code: 'invalid-format',
        });
    }
 ...
 *
 * Which formats those are is read from lib/limits.js and never restated. AVIF
 * rejoining the list in both directions needed no edit to the gates here for
 * exactly that reason.
 *
 * The second gate below stays. It is the thing that stops a format being added
 * to CONVERT_OUTPUT_FORMATS one day and quietly producing JPEG bytes under
 * somebody else's Content-Type, which is the same trap the sharp pipeline
 * called out in its own switch.
 *
 * AVIF IS THE ONE OUTPUT WHOSE FILE IS READ BACK BEFORE SUCCESS IS REPORTED.
 * Its encoder is a 842 KB binary loaded moments earlier, and its container is
 * one the visitor's phone may not open, so the header is parsed and the bytes
 * are decoded again before the job says it worked. See encodeAvifVerified.
 *
...
async function runConvert(context) {
    ...
    const encoded = format === 'avif'
        ? await encodeAvifVerified(context, pixels, {
            ...
            width: decoded.width,
            height: decoded.height,
            // AVIF keeps alpha, so an alpha item is expected exactly when the
            // decoder met a see-through pixel — and forbidden when it did not,
            // which catches an encoder inventing a plane.
            transparent,
        })
        : await encodeStage(context, pixels, { format, quality });
    ...
    return {
        ...
        viaNative: decoded.viaNative,
        transparent,
        // 8, 10 or 12 for an AVIF source and null for everything else. The
        // browser hands this engine 8-bit RGBA whatever the file held, so this
        // is the only place a 10-bit source can still be named — and the panel
        // says so rather than letting a person assume the depth survived.
        sourceBitDepth: decoded.sourceBitDepth ?? null,
    };
}
...
function resolveOptionalCrop(options, { sourceWidth, sourceHeight }) {
    ...
    if (![options.x, options.y, options.cropWidth, options.cropHeight].some(given)) {
        return { ok: true, rect: { x: 0, y: 0, width: sourceWidth, height: sourceHeight }, explicit: false };
    }

    const resolved = resolveCropRect(
        { x: options.x, y: options.y, width: options.cropWidth, height: options.cropHeight },
        { sourceWidth, sourceHeight },
    );

    // `explicit` says the rectangle came from the page rather than from the
    // whole image. /favicon-generator is the one caller that has to tell them
    // apart: it holds a FRAME somebody dragged to a square, and the whole image
...
/**
 * The two honest ways a mark that is not square reaches a square. There is no
 * stretch: an icon is the one picture on a site that is drawn at 16 pixels
 * beside other people's icons, and a squashed one reads as a broken one.
 */
const ICON_GEOMETRIES = ['cover', 'contain'];

/** The background value that means "leave the corners empty". */
const ICON_TRANSPARENT = 'transparent';

/** What a transparent margin is, as padImageData takes it. */
const ICON_CLEAR_PAD = { r: 0, g: 0, b: 0, a: 0 };

/** The only colours this op paints: the two named presets or a hex triplet/sextet. */
const ICON_HEX_COLOUR = /^#(?:[0-9a-f]{3}|[0-9a-f]{6})$/i;

function isIconBackground(value) {
    return BACKGROUND_PRESETS.some((preset) => preset.value === value) || ICON_HEX_COLOUR.test(value);
}

/**
 * Every square this op encodes, smallest first, DERIVED from the package
 * rather than typed beside it. 48 is in the list because favicon.ico holds it;
 * it is the one size that never becomes a loose file.
 */
const ICON_SIZES = [...new Set(ICON_ASSETS.flatMap(
    (asset) => asset.sizes ?? (asset.width ? [asset.width] : []),
))].sort((first, second) => first - second);

/** The largest surface this op will ever allocate. */
const LARGEST_ICON = ICON_SIZES[ICON_SIZES.length - 1];

const ICO_ASSET = ICON_ASSETS.find((asset) => asset.kind === 'ico');

/** The package minus the manifest, which is text the page builds from fields. */
const ICON_FILE_ASSETS = ICON_ASSETS.filter((asset) => asset.kind !== 'manifest');

/** Which file a size ends up in, for a refusal that names something real. */
function iconFileFor(size) {
    const png = ICON_ASSETS.find((asset) => asset.kind === 'png' && asset.width === size);
    return png ? png.filename : ICO_ASSET.filename;
}

/** '32 × 32' — how the check rows report a square. */
function describeSquare(size) {
    return `${size} × ${size}`;
}

/**
 * One icon: resample, trim or pad, composite if a colour was chosen, encode,
 * and then MEASURE THE RESULT.
 *
 * The measurement is the part that is not decoration. Everything else in this
 * function is arithmetic that has already been proved elsewhere, but a favicon
 * is the one output nobody looks at closely — a 16-pixel square in a tab strip
 * — and an icon that came out 31 pixels wide would be shipped, linked and
 * cached by a visitor who never noticed. readImageSize reads the finished PNG's
 * own IHDR and shares no code with the encoder that wrote it.
 */
async function encodeIcon(context, source, { size, geometry, colour }) {
    const { guard } = context;

    const plan = fitGeometry({
    ...
    if (!plan.ok) throw new JobError(plan.error, { code: 'invalid-dimensions' });

    let pixels = source;

    if (pixels.width !== plan.resampleTo.width || pixels.height !== plan.resampleTo.height) {
        const resized = await resizeImageData(pixels, plan.resampleTo);
        pixels = resized.data;
    }
    guard();

    if (plan.trim) {
        pixels = cropImageData(pixels, plan.trim);
        guard();
    }

    if (plan.pad) {
        pixels = padImageData(pixels, size, size, colour ? { ...colour, a: 255 } : ICON_CLEAR_PAD);
        guard();
    }

    // ONLY when a colour was chosen. A visitor who asked for a transparent
    // package asked for the mark's own alpha to survive, and PNG carries it —
    // so there is nothing here for flattening to fix and everything for it to
    // destroy.
    const flat = colour ? flattenImageData(pixels, colour) : pixels;
    guard();

    const encoded = await encodeImageData(flat, { format: 'png' });
    const bytes = new Uint8Array(await encoded.blob.arrayBuffer());

    const measured = readImageSize(bytes);
    if (!measured || measured.width !== size || measured.height !== size) {
        const actual = measured ? `${measured.width} × ${measured.height}` : 'unreadable';
        throw new JobError(
            `${iconFileFor(size)} came out ${actual} instead of ${describeSquare(size)}.`,
            { code: 'icon-size' },
        );
    }

    return { size, bytes, blob: encoded.blob, type: encoded.type, measured };
}

/**
 * /favicon-generator — one mark, six squares, and the container that holds
 * three of them.
 *
 * WHY IT IS ONE OP AND NOT SIX TRIPS THROUGH /resize
 *
 * The files in a favicon package have to agree with each other. favicon.ico
 * holds the very bytes of the 16 and the 32 rather than a second encode of
 * them; the manifest names the 192 and the 512 by filename; the snippet links
 * four of them. Six separate jobs would produce six correct files and no
 * guarantee that they are the same six the other three documents describe.
 *
 * WHY THE SOURCE IS DECODED ONCE AND CROPPED ONCE
 *
 * Six decodes of the same file is six times the most expensive stage in the
 * pipeline, and a frame re-resolved per size is a frame that can round
 * differently per size. So the picture is decoded, cropped to the frame and
 * then resampled six times from that one surface — which is also what makes
 * the six icons obviously the same picture.
 *
 * WHY THE MEMORY GATE IS AIMED AT 512. Five of the six squares are smaller than
 * a thumbnail. The only surface worth costing is the largest one, and it is
 * costed before anything is allocated, exactly as /passport-photo costs its
 * one output.
 */
async function runIcons(context) {
    const { options, report, guard } = context;

    const geometry = parseChoice(options.geometry, ICON_GEOMETRIES, ICON_GEOMETRIES[0]);
    if (geometry === null) {
        throw new JobError(`Invalid shape. Must be ${joinOr(ICON_GEOMETRIES)}.`, {
            code: 'invalid-geometry',
        });
    }

    // 'transparent' is this op's own value for a field every other tool uses
    // for a colour, and it is the DEFAULT: a mark arrives with its corners
    // empty far more often than not, and filling them uninvited is the one
    // thing that cannot be undone downstream.
    // Anything else is refused rather than painted: parseBackground falls
    // back to white for a value it does not know, and a package that came out
    // opaque because a form field was tampered with would look like a choice.
    const wanted = String(options.background ?? '').trim().toLowerCase();
    if (wanted !== '' && wanted !== ICON_TRANSPARENT && !isIconBackground(wanted)) {
        throw new JobError('Invalid background. Must be transparent, white, black or a hex colour.', {
            code: 'invalid-background',
        });
    }
    const colour = (wanted === '' || wanted === ICON_TRANSPARENT) ? null : parseBackground(wanted);
    ...
    // A contain keeps the whole picture by definition, so the frame is a
    // cover-only control and is ignored rather than half-applied.
    const rect = geometry === 'cover'
        ? crop.rect
        : { x: 0, y: 0, width: decoded.width, height: decoded.height };

    // One pixel of slack because the frame is dragged with a mouse and a
    // 300 x 301 rectangle is somebody asking for a square. Anything wider than
    // that is a page sending the wrong shape, and squeezing it into a square
    // would distort the mark in a way nothing downstream could detect.
    if (geometry === 'cover' && crop.explicit && Math.abs(rect.width - rect.height) > 1) {
        throw new JobError(
            `The frame must be square. It is ${rect.width} × ${rect.height} pixels.`,
            { code: 'invalid-crop' },
        );
    }

    const source = cropImageData(decoded.data, rect);
    guard();

    const largest = fitGeometry({
    ...
    if (!largest.ok) throw new JobError(largest.error, { code: 'invalid-dimensions' });

    const gate = assessPixels({
        sourceWidth: rect.width,
        sourceHeight: rect.height,
        // The biggest icon, which is the only one big enough to cost.
        targetWidth: LARGEST_ICON,
        targetHeight: LARGEST_ICON,
        intermediateWidth: largest.resampleTo.width,
        intermediateHeight: largest.resampleTo.height,
        fileBytes: Number(context.file.size) || 0,
        operation: 'icons',
        device: context.device,
        // Already upright and already cropped, so neither surface is charged twice.
    ...
    for (const [index, size] of ICON_SIZES.entries()) {
        encoded.set(size, await encodeIcon(context, source, { size, geometry, colour }));
        guard();

        const span = PROGRESS.encoded - PROGRESS.encoding;
        report(PROGRESS.encoding + Math.round((span * (index + 1)) / ICON_SIZES.length), 'encoding');
    }

    const ico = buildIco(ICO_SIZES.map((size) => ({ size, png: encoded.get(size).bytes })));

    // The engine reads its own container back before anybody is offered it. A
    // wrong offset here produces a file that shows a browser's default icon
    // and reports nothing, in any console, on any platform.
    let parsed;
    try {
        parsed = readIco(ico);
    } catch (error) {
        throw new JobError(`${ICO_ASSET.filename} did not read back correctly: ${error.message}`, {
            code: 'icon-package',
        });
    }

    const assets = ICON_FILE_ASSETS.map((asset) => {
        if (asset.kind === 'ico') {
            return {
                id: asset.id,
                filename: asset.filename,
                type: asset.type,
                blob: new Blob([ico], { type: asset.type }),
                bytes: ico.byteLength,
                // A container has no single size, and saying 48 would be a
                // claim the page would then print beside the wrong file.
                width: null,
                height: null,
                sizes: [...ICO_SIZES],
            };
        ...
        const icon = encoded.get(asset.width);
    ...
    const checks = ICON_FILE_ASSETS.map((asset) => {
        if (asset.kind === 'ico') {
            const required = ICO_SIZES.join(', ');
            const actual = parsed.entries.map((entry) => entry.width).join(', ');
            return { key: asset.filename, required, actual, ok: actual === required };
        }

        const { measured } = encoded.get(asset.width);
        const required = describeSquare(asset.width);
        const actual = `${measured.width} × ${measured.height}`;

        return { key: asset.filename, required, actual, ok: actual === required };
    });

    const failed = checks.find((row) => !row.ok);
    if (failed) {
        throw new JobError(`${failed.key} came out ${failed.actual} instead of ${failed.required}.`, {
            code: 'icon-package',
        });
    }

    // What was enlarged to make the 512, if anything — from the same function
    // the page warns with before the job, so the two never disagree by value.
    const enlargedFrom = iconEnlargedFrom({
        sourceWidth: rect.width,
        sourceHeight: rect.height,
        geometry,
        largest: LARGEST_ICON,
    });

    report(PROGRESS.encoded, 'encoded');

    return {
        // A package is not one file. The result wrapper reads `assets` and
        // leaves `blob` null rather than picking one of the six to be special.
        blob: null,
        assets,
        checks,
        verified: checks.every((row) => row.ok),
        bytes: assets.reduce((sum, asset) => sum + asset.bytes, 0),
        format: 'png',
        type: encoded.get(ICON_SIZES[0]).type,
        width: null,
        height: null,
        crop: rect,
        fit: geometry,
...
const OPERATIONS = {
    ...
    /**
     * /favicon-generator. The one op that answers with a PACKAGE rather than a
     * file: six images, and the result wrapper leaves `blob` null because none
     * of the six is the answer on its own. Wrapped in withJobErrors for the
     * same reason the two above are — the ICO writer is a leaf that cannot
     * build a JobError without importing this module back.
     */
    icons: {
        accept: RASTER_INPUT_FORMATS,
        invalidTypeError: 'File failed validation. Please upload a valid JPEG, PNG, or WebP image.',
        prefix: 'resizo-favicon',
        run: withJobErrors(runIcons),
    },
...
async function prepareEntry(source, options, { spec, operation, device, sourceWidth, sourceHeight }) {
    ...
    const gate = assessFile(file, {
        operation,
        sourceWidth,
        sourceHeight,
        orientation,
        outputFormat: costedOutputFormat(options),
        device,
    });
    if (!gate.ok) {
...
export async function runOperation(operation, source, options = {}, {
    ...
    return {
        // /favicon-generator answers with a package. None of its six files is
        // the result on its own, so there is no blob to hand a download button
        // and pretending otherwise would put one of them behind the main
        // action at random.
        blob: outcome.assets ? null : outcome.blob,
        // The files a package op produced, in package order, each with its own
        // blob and byte length. Null on every other op.
        assets: outcome.assets ?? null,
        filename,
        operation,
        format: outcome.format,
        type: outcome.type,
        ...
        // AVIF only. The depth the SOURCE declared, so a page can say "10-bit
        // source decoded to 8-bit" instead of implying the depth survived, and
        // the encoder heap the worker lifecycle is decided on.
        sourceBitDepth: outcome.sourceBitDepth ?? null,
        avifHeapBytes: outcome.avifHeapBytes ?? null,
        ...
        // /favicon-generator only: the square the icons were made from, when it
        // is smaller than the largest icon in the package. Null when the source
        // had pixels to spare, which is the common case.
        enlargedFrom: outcome.enlargedFrom ?? null,
