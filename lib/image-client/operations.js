...
async function runIcons(context) {
    ...
    const colour = (wanted === '' || wanted === ICON_TRANSPARENT) ? null : parseBackground(options.background);
    ...
    // The square the icons are actually made of: a cover keeps the largest
    // square INSIDE the frame, a contain fits the whole picture inside one. It
    // is the number that decides whether 512 is a blow-up, and the page says so
    // in as many words rather than silently enlarging.
    const square = geometry === 'cover'
    ...
    return {
        ...
        originalHeight: decoded.height,
        viaNative: decoded.viaNative,
        enlargedFrom: square < LARGEST_ICON ? { width: square, height: square } : null,
