 ...
 * renderer cannot see.
 *
 * A GUIDE MAY NEVER LAND ON A PHOTO. The layout already drops any mark whose
 * rectangle would, but the compositor masks every guide pixel against the cells
 * anyway. Two independent guards, because a grey line through somebody's face
 * is not a cosmetic defect — it is a photo they cannot use, printed on paper
 * they paid for.
 *
 * THE RECTANGLES ARE THE LAYOUT'S, NOT THIS MODULE'S. `guideRects` and
 * `referenceRects` say where the ink goes, in paper pixels, and the PDF writer
 * and the SVG preview read the same two functions. This module used to grow a
 * mark into a rectangle itself, which is how the PDF ended up centring its
 * strokes somewhere else and printing a 50 mm line that measured 50.4.
 *
...
import { describeSheetSize, guideRects, referenceRects } from '@/lib/format/print-sheet';
...
export function composeSheet(layout, photo) {
    ...
    for (const rect of guideRects(layout)) {
        fillMasked(canvas, rect, GUIDE_COLOUR, layout.cells);
    }

    for (const rect of referenceRects(layout)) {
        fillMasked(canvas, rect, REFERENCE_COLOUR, layout.cells);
    }
...
function pdfChecks(bytes, layout, pages) {
    ...
    return [
        ...
        check(
            'dpi',
            // Not 'Resolution', which is what the JPEG row is called: that one
            // is read out of the file's own record and this one cannot be,
            // because there is no record in a PDF to read.
