...
/**
 * AVIF is the one format here whose header is a whole box tree rather than a
 * fixed field, so the walk lives in its own leaf — lib/image-client/avif.js —
 * and this branch takes the two rows the contract asks for. That module reads
 * more than this (brands, animation, chroma, bit depth, orientation), and every
 * one of those has a caller that is NOT a size check, which is why the reader
 * is not folded in here.
 *
 * A file whose `ispe` could not be reached reads as null rather than as a
 * picture of unknown size: a validator handed "width: null" would compare it
 * against a real number and pass.
 */
function avifSize(bytes) {
    const header = readAvifHeader(bytes);
    if (!header || header.width === null || header.height === null) return null;

    return { width: header.width, height: header.height, hasAlpha: header.alpha };
}
...
export function readImageSize(input) {
    ...
    try {
        ...
        if (format === 'avif') return avifSize(bytes);
    } catch {
