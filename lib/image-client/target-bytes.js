 ...
 *
 * This is the same algorithm as lib/image/target-size.js, and it is here as a
 * SEPARATE COPY for one reason: target-size.js imports lib/image/pipeline.js,
 * which imports sharp. One import of it from a worker drags a native Node
 * addon into a browser bundle and the build dies. The parse, the bounds and
 * the two user-facing sentences are reproduced verbatim so a request the server
 * refused is refused here in the same words during the changeover.
 *
 *   FOLLOW-UP: when the server routes are deleted, parseTargetBytes,
 *   bytesToKb and impossibleTargetMessage should collapse back into one pure
 *   module that both callers import. They are duplicated only for as long as
 *   both engines are alive.
 *
 * WHAT IS DIFFERENT FROM THE SERVER, AND WHY
 *
 *  - The search is fed a DECODED ImageData, not a file. The server re-decoded
 *    the source on every probe (sharp clones a pipeline); here the source is
 *    decoded once and only the encode repeats. Eight probes therefore cost
 *    eight encodes and one decode instead of eight of each.
 *
 *  - PNG skips the quality phase entirely. On the server, phase one quantises
 *    the palette and phase two drops pixels. @jsquash/png has no quantiser at
 ...
 *
 *  - The deadline is shorter. The server's 35 s existed to beat a platform 504;
 *    here there is no gateway, only a person holding a phone.
 */
...
/**
 * Largest 1-100 quality whose encode still fits the target.
 *
 * @param {object} input
 * @param {(quality: number) => Promise<{ bytes: number, payload: * }>} input.probe
 */
export function searchQuality({ probe, targetBytes, maxIterations, deadline, now, onIteration }) {
    return searchRange({ probe, low: 1, high: 100, targetBytes, maxIterations, deadline, now, onIteration });
}
