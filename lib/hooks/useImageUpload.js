...
import { AVIF_ANIMATED_MESSAGE, AVIF_DAMAGED_MESSAGE, AVIF_HEADER_SCAN_BYTES, readAvifHeader } from '@/lib/image-client/avif';
import { canDecodeAvif } from '@/lib/image-client/capability';
...
/**
 * A file that sniffs as AVIF is still not necessarily one this tab can open —
 * canDecodeAvif() decodes a tiny built-in AVIF once with createImageBitmap and
 * remembers the answer for this browser. Checked here, at the point of drop,
 * rather than after a Convert/Resize button has already been pressed: a
 * refusal at intake reads like every other rejectReason, next to the file the
 * visitor just chose.
 */
const AVIF_UNSUPPORTED_BROWSER_MESSAGE = 'This browser cannot open AVIF images. Chrome 85, Firefox 93, and '
    + 'Safari 16 on iOS 16 or macOS Ventura and later can.';
...
/**
 * An AVIF's pixel size from the browser's own decoder: `{ width, height }`,
 * `false` when the decoder refused the file, `null` where the browser has no
 * createImageBitmap (the caller then measures it the ordinary way).
 */
async function measureAvif(file) {
...
/** The AVIF container's own facts, from the bytes a header can live in. */
async function readAvifHeaderOf(file) {
    try {
        const head = await file.slice(0, AVIF_HEADER_SCAN_BYTES).arrayBuffer();
        return readAvifHeader(new Uint8Array(head));
    } catch {
        return null;
    }
}
...
export function useImageUpload({
    ...
    const buildEntry = useCallback(async (file) => {
        ...
        // Only ever asked about a file this tool has already agreed to take —
        // a tool with no 'avif' in its accept list refused it above, in its own
        // words, and never reaches this browser-support question at all.
        if (sniffed === 'avif' && !(await canDecodeAvif())) {
            return { error: AVIF_UNSUPPORTED_BROWSER_MESSAGE };
        }

        // The container is read before any preview exists. A browser handed an
        // animated AVIF would show its first frame and say nothing, and one
        // handed a broken container would fail a decode with a sentence about
        // "an image"; both are refused here by name, from the header, so no
        // object URL and no <img> probe is ever made for them. The engine
        // repeats the same read before it decodes, with the same two sentences.
        let decodedAvif = null;
        if (sniffed === 'avif') {
            const header = await readAvifHeaderOf(file);
            // Every AVIF image item must carry its size (ispe); a container
            // with no readable size is not one a browser will open.
            if (!header || header.width === null || header.height === null) return { error: AVIF_DAMAGED_MESSAGE };
            if (header.animated) return { error: AVIF_ANIMATED_MESSAGE };

            // Measured with the decoder the engine will use, from the bytes,
            // before any object URL exists. A broken picture fails HERE, in
            // this hook's own words — and never as an <img> whose load fails,
            // which a Playwright trace then tries to fetch back through a
            // blob: URL the site's connect-src forbids. Where the browser has
            // no createImageBitmap the <img> probe below still measures it.
            decodedAvif = await measureAvif(file);
            if (decodedAvif === false) return { error: AVIF_DAMAGED_MESSAGE };
        }
        ...
        if (previewUrl) {
            const measured = decodedAvif ?? await readDimensions(previewUrl);
            if (!measured) {
                revoke(previewUrl);
                // An AVIF whose header parsed but whose picture the browser
                // will not decode (a truncated download, a corrupt payload)
                // is named as an AVIF, not as "an image".
                return { error: sniffed === 'avif' ? AVIF_DAMAGED_MESSAGE : rejectReason.unreadable() };
            }
