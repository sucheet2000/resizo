...
export function useLocalProcess({ op, autoDownload = false, onSuccess } = {}) {
     ...
     *   which is the one thing the gate exists to get in front of.
    ...
    const submit = useCallback((input, {
        ...
        const assessment = assessJob(file, {
            operation: op,
            sourceWidth: rest.sourceWidth,
            sourceHeight: rest.sourceHeight,
            targetWidth: rest.targetWidth,
            targetHeight: rest.targetHeight,
        });

        if (!assessment.ok) {
            return Promise.resolve(fail(refusalMessage(assessment), assessment.suggestion ?? null));
        }
