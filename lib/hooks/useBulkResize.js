...
export function useBulkResize({ op = 'resize' } = {}) {
    ...
    const run = useCallback(async (items) => {
        ...
        // The archive ceiling stopped it before the files ran out. That is not
        // a failure — a ZIP was built and some images are in it — so it must
        // not be reported as one, and it must not be silent either: the whole
        // point of stopping was to say so instead of letting the tab die.
        setError(outcome.leftOutMessage ?? ALL_FAILED_MESSAGE);
        return null;
    }, [processFile]);

    const progress = useMemo(() => {
        if (progressRows.length === 0) return 0;
        // 'skipped' counts as settled. A batch stopped by the archive ceiling
        // leaves real rows in that state, and without them here the bar stalls
        // short of 100% on a run that has genuinely finished.
        const settled = progressRows.filter(
            (row) => row.status === 'done' || row.status === 'failed' || row.status === 'skipped',
        ).length;
