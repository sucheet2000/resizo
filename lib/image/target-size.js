...
function encode(buffer, format, quality) {
    return applyOutputFormat(createPipeline(buffer), format, { quality }).toBuffer();
}
 ...
 */
async function searchQuality({ buffer, format, targetBytes, maxIterations }) {
    let low = 1;
    let high = 100;
    let fit = null;
    let fitQuality = null;
    ...
    while (low <= high && iterations < maxIterations) {
        ...
        const output = await encode(buffer, format, quality);
        iterations += 1;
 ...
 */
async function searchScale({ buffer, targetBytes, quality, maxIterations }) {
    let metadata;
    try {
        metadata = await createPipeline(buffer).metadata();
    } catch {
    ...
    while (low <= high && iterations < maxIterations) {
        ...
        const resized = createPipeline(buffer).resize({
...
export async function compressToTarget({
    ...
    const quality = await searchQuality({ buffer, format, targetBytes, maxIterations });
    ...
    const scale = await searchScale({ buffer, targetBytes, quality: 1, maxIterations });
