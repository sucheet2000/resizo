 ...
 *
 * AVIF: no longer reachable. It left CONVERT_OUTPUT_FORMATS along with every
 * other allowlist, so nothing can hand this function 'avif' any more. The case
 * stays anyway, because the alternative is worse than dead code: without it
 * 'avif' would fall through to the jpeg branch, and a caller that somehow got
 * the string this far would be served JPEG bytes under an image/avif
 * Content-Type. An unreachable honest encoder beats a reachable lie.
 */
