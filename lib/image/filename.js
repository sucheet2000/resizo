...
const EXTENSIONS = new Set(['png', 'webp', 'gif', 'heic', 'heif', 'avif', 'tiff', 'zip']);
...
/**
 * Builds a Content-Disposition value carrying both the legacy ASCII form and
 * the RFC 5987 UTF-8 form. Quotes, backslashes, semicolons, commas and every
 * control character (CR/LF included) are removed from the ASCII form and
 * percent-encoded in the UTF-8 form, so neither can break out of the header.
 */
export function contentDispositionValue(filename, { disposition = 'attachment' } = {}) {
    const raw = typeof filename === 'string' && filename.trim() ? filename.trim() : 'download';
