...
/**
 * Builds '<prefix>-<base>[-<suffix>].<ext>' from the uploaded filename. A
 * `name` of null says the output is a new document with no source to be named
 * after — a print sheet — and the base is left out rather than falling back.
 */
export function buildOutputFilename({ name, prefix, format, suffix } = {}) {
    const base = name === null ? '' : sanitizeBaseName(name);
    const parts = [sanitizeToken(prefix), base, sanitizeToken(suffix)];
