 ...
 *
 * Mirrors resolveExplicitTarget in app/api/resize/route.js case for case,
 * including the part that is easy to miss: the caps apply to the side that was
 * DERIVED as well as the side that was supplied. width=8000 on a tall thin
 * source otherwise asks for a height in the hundreds of thousands, and both the
 * server and the browser have to refuse that with the same sentence.
 *
 * Both sides supplied means exactly those two numbers, stretch included — which
 * is what sharp's default `fit: 'cover'` produces and what the browser engine
 * reproduces.
 *
