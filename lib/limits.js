...
// The list twenty consumers read: /crop, /compress, the signature resizer, the
// print sheet, the requirement fitter, the icon package, /jpg-to-pdf and the
// two bulk tools. AVIF is deliberately NOT here. Every one of those tools ends
// at a form, a printer or a favicon, and widening the shared list would put
// AVIF into all twenty at once for the sake of the two that can use it.
...
// What /bulk-image-converter can write, and the one list that is narrower than
// CONVERT_OUTPUT_FORMATS purely for memory. One AVIF encode is fine; twenty in
...
// /convert, where one file is one encode and the worker is recycled after it.
export const BULK_CONVERT_OUTPUT_FORMATS = ['jpeg', 'png', 'webp'];
