            ...
            >
                ...
                <ContentSection id="what-200kb-buys" heading="What 200 KB actually buys you">
                    <p>
                        ...
                        &ldquo;will it still look good&rdquo;, and it depends on the dimensions rather than on
                        the compressor.
                    </p>
                    <div className="overflow-x-auto">
                        <table className="w-full min-w-[32rem] border-collapse text-left">
                            <caption className="sr-only">
                                Bits available per pixel at a 200 KB target, by image size
                            </caption>
                            <thead>
                                <tr className="border-b border-line">
                                    <th scope="col" className="py-2 pr-4 text-ui text-ink">Image size</th>
                                    <th scope="col" className="py-2 pr-4 text-ui text-ink">Pixels</th>
                                    <th scope="col" className="py-2 pr-4 text-ui text-ink">Bits each</th>
                                    <th scope="col" className="py-2 text-ui text-ink">What to expect</th>
                                </tr>
                            </thead>
                            <tbody>
                                {BUDGET.map((row) => (
                                    <tr key={row.size} className="border-b border-line align-top">
                                        <th scope="row" className="py-2 pr-4 font-data text-ui font-medium text-ink">
                                            {row.size}
                                        </th>
                                        <td className="py-2 pr-4 font-data text-ui">{row.pixels}</td>
                                        <td className="py-2 pr-4 font-data text-ui">{row.bits}</td>
                    ...
                    <p>
                        As a rule of thumb, a JPEG at about one bit per pixel is hard to tell from the
                ...
                <ContentSection id="where-the-limit-comes-from" heading="Where a 200 KB limit usually comes from">
                    <p>
                        Unlike a 100 KB cap, which is nearly always imposed on you by a form, 200 KB is often a
                    ...
                    <p>
                        It also turns up as a hard limit in the same places 100 KB does — forum avatars and
                ...
                <ContentSection id="versus-100kb" heading="200 KB or 100 KB?">
                    <p>
                        If nothing is forcing your hand, take the 200 KB. Halving the budget means the same
                    ...
                    <p>
                        If a form does force your hand, the{' '}
                        <Link
                        ...
                        is set up for it, and the trick there is to cut the pixel dimensions before compressing
                        rather than asking the encoder for the impossible. For any other number, the target
                        field above accepts anything from 10 KB to 20 MB, and the{' '}
                        <Link
                            href="/compress"
                            className="rounded-input font-medium text-accent underline underline-offset-4"
                        >
                ...
                <FaqList items={FAQS} id="compress-200kb-faq" />
            </CompressTool>
        </>
    );
}
