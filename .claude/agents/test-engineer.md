...
- Structural rules are tests too. `tests/architecture/boundaries.test.js` (import graph: React out of the worker, no upward `lib/` import, engine away from the site catalogue, heavy packages behind `import()`, no cycles, no client module reaching the catalogue barrel or the intent copy), `tests/architecture/no-dead-code.test.js` (every module reachable, clean repo root, no file named after a sibling directory) and `tests/design/contract.test.js` (banned copy and raw palette utilities) are the suites that fail on things a unit test cannot see. Never satisfy any of them by loosening an assertion or adding an exception entry — that is the one wrong answer. When you add an architecture assertion, prove it RED by introducing the violation, then revert, and report what you introduced.
- **A test is not evidence that its subject is used.** Four dead modules in this repo each carried a green, thorough unit test, and coverage counted every line — the only thing importing them was the test. So when you audit coverage, ask reachability first: if nothing under `app/`, `scripts/` or the worker reaches a module, more tests on it are waste and the module is the bug. Rule 6 in CLAUDE.md automates the check; do not hand-wave past a failure by giving the orphan an importer.
- Every guard needs a self-check that proves it read something, or it passes vacuously. An empty import graph violates no boundary and an empty file list contains no stray — assert the walk found real modules before asserting what it did not find.
