...
- **`tests/architecture/` is the mechanism, not the prose.** Nine boundaries are executable. `boundaries.test.js` holds the six about the import graph — no React in the worker's graph, no `lib/` → `app/`/`components/` edge, no `lib/image-client/` → `lib/catalog/` edge, heavy packages only behind `import()`, no static cycle in `lib/`, no `'use client'` module reaching the catalogue barrel or the intent copy. `no-dead-code.test.js` holds the three about what survives in the tree — every `lib/` and `components/` module reachable from an entry point, no stray file in the repo root, no `lib/` file named after a directory beside it. Run both before you rule on a structural change. If you decide a NEW boundary is worth having, it lands as an assertion in one of those files in the same change, or it does not land: a rule that only exists in markdown is a rule that gets broken by the next reasonable-looking import.
- **Deletion is part of structure.** A module nothing reaches is not free — it is read, trusted, imitated and maintained. Rule 6 exists because four dead modules each kept a passing test, so the suite reported dead code as covered working code; **a test importing a dead module makes it look alive, and coverage will never tell you.** When you find one, the fix is to delete it, not to find it a caller. Deleting a source file needs the owner's explicit confirmation — name the files and the evidence, and let him say go.
...
- **A name that no longer tells the truth is a structural defect**, and it is the one kind you must raise rather than fix silently: renaming a high-fan-in directory is a large diff for a benefit no test measured. Propose it with the import-site count and let the owner decide.
