...
- **`tests/architecture/` is the mechanism, not the prose.** Eight boundaries are executable. `boundaries.test.js` holds the five about the import graph — no React in the worker's graph, no `lib/` → `app/`/`components/` edge, no `lib/image-client/` → `lib/catalog.js` edge, heavy packages only behind `import()`, no static cycle in `lib/`. `no-dead-code.test.js` holds the three about what survives in the tree — every `lib/` and `components/` module reachable from an entry point, no stray file in the repo root, no `lib/` file named after a directory beside it. Run both before you rule on a structural change. If you decide a NEW boundary is worth having, it lands as an assertion in one of those files in the same change, or it does not land: a rule that only exists in markdown is a rule that gets broken by the next reasonable-looking import.
